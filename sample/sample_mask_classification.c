...
static int GetVideoframe(VIDEO_FRAME_INFO_S *stfdFrame) {
...
static int ReleaseVideoframe(VIDEO_FRAME_INFO_S *stfdFrame) {
  int s32Ret = CVI_SUCCESS;
...
static void Run() {
  ...
  while (bExit == false) {
    s32Ret = GetVideoframe(&stfdFrame);
    if (s32Ret != CVI_SUCCESS) {
      Exit();
...
static CVI_S32 InitVPSS() {
  ...
  abChnEnable[VpssChn] = CVI_TRUE;
  stVpssChnAttr[VpssChn].u32Width = 640;
  stVpssChnAttr[VpssChn].u32Height = 480;
