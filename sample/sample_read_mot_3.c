...
int main(int argc, char *argv[]) {
  ...
  CVI_AI_SelectDetectClass(ai_handle, model_config.model_id, 2, CVI_AI_DET_TYPE_DOG,
                           CVI_AI_DET_TYPE_CAT);
  ...
  FILE *outFile;
  outFile = fopen("sample_MOT_result_3.txt", "w");
  if (outFile == NULL) {
  ...
  for (int counter = 0; counter < imageNum; counter++) {
    ...
    *strchrnul(line, '\n') = '\0';
    ...
    for (uint32_t i = 0; i < obj_meta.size; i++) {
      // if (obj_meta.info[i].classes == 16 || obj_meta.info[i].classes == 17) counter += 1;
      // if (obj_meta.info[i].classes == 16) counter += 1;
      if (obj_meta.info[i].classes == 17) counter += 1;
    }
    ...
    counter = 0;
    for (uint32_t i = 0; i < obj_meta.size; ++i) {
      // if (obj_meta.info[i].classes == 16 || obj_meta.info[i].classes == 17){
      // if (obj_meta.info[i].classes == 16){
      if (obj_meta.info[i].classes == 17) {
