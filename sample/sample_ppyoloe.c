...
int main(int argc, char* argv[]) {
  ...
  if (argc != 3) {
    printf("Usage: %s <ppyoloe model path> <input image path>\n", argv[0]);
    printf("ppyoloe model path: Path to ppyoloe model cvimodel.\n");
  ...
  ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_PPYOLOE, argv[1]);
  if (ret != CVI_SUCCESS) {
