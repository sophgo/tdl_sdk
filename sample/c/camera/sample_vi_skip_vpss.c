...
void *process_thread(void *arg) {
  ...
  while (to_exit == false) {
    ...
    void *extracted_frame = NULL;
    ret = TDL_WrapImage(image, &extracted_frame);

    printf("Successfully extracted frame from image\n");

    // Now, convert the frame back to image using TDL_WrapFrame
    TDLImage re_wrapped_image = TDL_WrapFrame(extracted_frame, false, true);
    // Note: Assuming TDL_Detection is thread-safe relative to GetCameraFrame
    ret = TDL_Detection(args->tdl_handle, args->model_id, re_wrapped_image,
                        &obj_meta);
    if (ret != 0) {
