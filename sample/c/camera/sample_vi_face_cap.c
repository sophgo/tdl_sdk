...
void print_usage(const char *prog_name) {
  printf("Usage:\n");
  printf("  %s -c <config_file> -o <output_dir> -v <vi_chn>\n", prog_name);
  printf(
      "  %s --config_file <path> --output_dir <dir> "
      "--vi_chn <int> \n",
      prog_name);
  printf("Options:\n");
  printf(
...
void *run_tdl_thread(void *args) {
  ...
  while (to_exit == false) {
    for (size_t i = 0; i < pstArgs->channel_size; i++) {
      ...
      if (frm_diff > 30) {
        printf("detect person size: %d, face size: %d\n",
               capture_info.person_meta.size, capture_info.face_meta.size);
      }

      for (uint32_t j = 0; j < capture_info.snapshot_size; j++) {
        if (capture_info.snapshot_info[j].object_image) {  // save snapshot
          char filename[512];
          sprintf(filename, "%s/%" PRIu64 "_face_%" PRIu64 ".jpg",
                  pstArgs->output_dir,
          ...
          ret = TDL_EncodeFrame(
              pstArgs
                  ->tdl_handle,  // face_cap_app_vi.json 中
                                 // vechn为-1时会自动另外创建vechn=1的编码通道
              capture_info.snapshot_info[j].object_image, filename, 1);
          if (ret != 0) {
...
int main(int argc, char *argv[]) {
  char *config_file = NULL;  // sample/c/config/face_cap_app_vi.json
  ...
  ret = TDL_APP_Init(tdl_handle, "face_capture", config_file, &channel_names,
                     &channel_size, false);
  if (ret != 0) {
