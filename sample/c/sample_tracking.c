...
void print_usage(const char *prog_name) {
    ...
    printf("  -i, --input       Path to input images dir, such as \"-i input\",\n"
...
int main(int argc, char **argv) {
    ...
    *comma = '\0';  
    ...
    ret = TDL_OpenModel(tdl_handle, model_id_face, model1);
    if (ret != 0) {
      printf("open face detection model failed with %#x!\n", ret);
      goto exit0;
    }
  
    ret = TDL_OpenModel(tdl_handle, model_id_obj, model2);
    if (ret != 0) {
    ...
    for (int i = 0; i < image_num; i++) {
        ...
        if (track_meta.out_num > 0) {
            ...
            for (int c = 0; c < track_meta.out_num; c ++) {
                printf("frame_id = %d, obj: %d, track_id = %d, box = [%f, %f, %f, %f]\n",
                        i, c,
                        track_meta.info[c].id,
                        track_meta.info[c].bbox.x1,
                        track_meta.info[c].bbox.y1,
                        track_meta.info[c].bbox.x2,
