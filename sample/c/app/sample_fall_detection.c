...
int main(int argc, char *argv[]) {
  ...
  int ret = TDL_APP_Init(tdl_handle, "fall_detection", config_file,
  ...
  while (true) {
    for (size_t i = 0; i < channel_size; i++) {
      ...
      ret = TDL_APP_FallDetection(tdl_handle, channel_names[i], &capture_info);
      if (ret == 1) {
        continue;
      } else if (ret == 2) {
        to_exit = true;
      ...
      if (capture_info.person_meta.size > 0) {
        colors = (int *)malloc(capture_info.person_meta.size * 3 * sizeof(int));

        for (uint32_t j = 0; j < capture_info.person_meta.size; j++) {
          boxes[j].x1 = capture_info.person_meta.info[j].box.x1;
          boxes[j].y1 = capture_info.person_meta.info[j].box.y1;
          boxes[j].x2 = capture_info.person_meta.info[j].box.x2;
          boxes[j].y2 = capture_info.person_meta.info[j].box.y2;

          if (capture_info.person_meta.info[j].falling) {
            colors[j * 3] = 0;
            colors[j * 3 + 1] = 0;
            colors[j * 3 + 2] = 255;  // 红色
            printf("person is falling, box: (%.2f, %.2f, %.2f, %.2f)\n",
      ...
      ret = VisualizeRectangle(boxes, capture_info.person_meta.size,
                               capture_info.image, outpath, colors);
      if (ret != 0) {
