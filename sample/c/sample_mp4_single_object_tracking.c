...
void print_usage(const char *prog_name) {
  ...
  printf(
      ...
      "<save_dir> -T <target_search_type> [-f <sam_model_path>] [-k <0|1>]\n",
      prog_name);
  printf(
      "  %s --video_path <path> --sot_model_path <path> --txt_init_path <path> "
      "--save_dir <dir> --target_search_type <0-3> [--sam_model_path <path>] "
      "[--use_kalman <0|1>]\n",
      prog_name);
  printf("Options:\n");
  printf(
      ...
      "  -k, --use_kalman : 1 to enable kalman filter, 0 to disable (default "
      "0)\n"
      "  -h, --help : print help\n");
}
...
int main(int argc, char *argv[]) {
  ...
  struct option long_options[] = {
      ...
      {"use_kalman", required_argument, 0, 'k'},
      {"help", no_argument, 0, 'h'},
  };

  int opt;
  while ((opt = getopt_long(argc, argv, "v:m:t:s:T:f:k:h", long_options,
                            NULL)) != -1) {
    switch (opt) {
      case 'v':
        video_path = optarg;
  ...
  if (!video_path || !sot_model_path || !txt_init_path || !save_dir ||
      target_search_type < 0) {
    fprintf(stderr,
            "Error: video_path, sot_model_path, txt_init_path, save_dir and "
            "target_search_type are required\n");
    print_usage(argv[0]);
    return -1;
  }
  ...
  // Disable Kalman filter by default to align with official python
  // implementation
  TDL_SetSingleObjectTrackingUseKalman(tdl_handle, use_kalman == 1);

  ret = TDL_SetSingleObjectTracking(
      tdl_handle, image, &obj_meta, values, 4, g_frame_id,
      (TDLTargetSearchTypeE)target_search_type, model_path);
...
#ifdef __BM168X__
  void *video_writer = NULL;
  char mp4_path[256];
  snprintf(mp4_path, sizeof(mp4_path), "%s/result.mp4", save_dir);
#endif
...
#ifdef __BM168X__
  if (video_writer) {
    SaveVideo_Release(video_writer);
  }
#endif
