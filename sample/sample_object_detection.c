...
static int GetVideoframe(VIDEO_FRAME_INFO_S *obj_det_frame, VIDEO_FRAME_INFO_S *video_output_frame)
{
	int ret = CVI_SUCCESS;
...
static int ReleaseVideoframe(VIDEO_FRAME_INFO_S *obj_det_frame, VIDEO_FRAME_INFO_S *video_output_frame)
{
	int ret = CVI_SUCCESS;
...
static CVI_S32 InitVI(SAMPLE_VI_CONFIG_S* video_input_config)
{
	...
	ret = SAMPLE_COMM_VI_GetSizeBySensor(video_input_config->astViInfo[work_sns_id].stSnsInfo.enSnsType, &pic_size);
	if (ret != CVI_SUCCESS) {
	...
	block_size = COMMON_GetPicBufferSize(stSize.u32Width, stSize.u32Height, SAMPLE_PIXEL_FORMAT,
										 DATA_BITWIDTH_8, video_input_config->astViInfo[work_sns_id].stChnInfo.enCompressMode,
										 DEFAULT_ALIGN);
	block_rot_size = COMMON_GetPicBufferSize(stSize.u32Height, stSize.u32Width, SAMPLE_PIXEL_FORMAT,
											DATA_BITWIDTH_8, video_input_config->astViInfo[work_sns_id].stChnInfo.enCompressMode,
											DEFAULT_ALIGN);
	block_size = MAX(block_size, block_rot_size);
	block_rgb_size = COMMON_GetPicBufferSize(vpss_group_width, vpss_group_height, PIXEL_FORMAT_RGB_888,
											DATA_BITWIDTH_8, video_input_config->astViInfo[work_sns_id].stChnInfo.enCompressMode,
											DEFAULT_ALIGN);
