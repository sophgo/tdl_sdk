...
int main(int argc, char *argv[]) {
  ...
  CVI_TDL_Set_ROI(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, &bg, roi_s, PIXEL_FORMAT_RGB_888,
                  &crop_frame);

  CVI_TDL_Detection(tdl_handle, crop_frame, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, &obj_meta);

  if (ret != 0) {
    printf("CVI_TDL_Detection failed with error code: %d\n", ret);
  }

  CVI_TDL_Release_VideoFrame(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, crop_frame, true);
