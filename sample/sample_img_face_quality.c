...
int process_image_file(cvitdl_handle_t tdl_handle, const char *imgf, cvtdl_face_t *p_obj) {
  ...
  if (p_obj->size > 0) {
    ret = CVI_TDL_FaceQuality(tdl_handle, &bg, p_obj, NULL);
    if (ret != CVI_SUCCESS) {
      printf("CVI_TDL_SUPPORTED_MODEL_FACEQUALITY failed with %#x!\n", ret);
      return ret;
    }
  } else {
...
int main(int argc, char *argv[]) {
  ...
  ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_FACEQUALITY, ln_model);
  if (ret != CVI_SUCCESS) {
    printf("open CVI_TDL_SUPPORTED_MODEL_FACEQUALITY model failed with %#x!\n", ret);
    return ret;
  }
