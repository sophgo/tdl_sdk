...
void signal_handler(int signum) {
  ...
  for (int i = 0; i < MAX_THREAD_NUM; i++) {
    if (g_cap[i].isOpened()) {
      cout << "thread " << i << "timed out! Force to release video capture!"
           << endl;
      g_cap[i].release();
    }
  }
...
#endif
{
  ...
  while (!g_exit_flag) {
    ...
    if (dis_mode == 1) {
      for (int i = 0; i < thread_num; i++) {
        if (i == 0)
          printf(
              "ID[%d], [%5d  %5d],  RECON[%5lld], FRM[%10lld], "
              "FPS[%2.2f],[%2.2f] drop[%9lld] |   FRM[%10lld], "
              "FPS[%2.2f],[%2.2f] enc_que_nums[%d] \n",
              i, g_width_enc_in[i], g_height_enc_in[i], g_reconnect_count[i],
              (long long)count[i],
              ((double)(count[i] - last_count[i])) / INTERVAL, fps[i],
              g_drop_count[i], (long long)count_enc[i],
              ((double)(count_enc[i] - last_count_enc[i])) / INTERVAL,
              fps_enc[i], g_image_enc_queue_nums[i]);
        else
          printf(
              "ID[%d] ,[%5d  %5d],  RECON[%5lld], FRM[%10lld], "
              "FPS[%2.2f],[%2.2f] drop[%9lld] |   FRM[%10lld], "
      ...
      for (int i = 0; i < thread_num; i++) count_sum += count[i];
      printf("thread %d, frame %lld, fps %2.2f", thread_num, count_sum,
  ...
  printf(
      "\n[ID],[frame nums], [rate],  [invl>80ms],  [invl>160ms], [invl>400ms], "
      "[invl>1s]\n");
  for (int i = 0; i < thread_num; i++) {
    printf("[%d], [%10lld], [%2.2f], [%10llu], [%10llu], [%10llu], [%10llu]\n",
           i, (long long)count[i], fps[i], g_interval_80[i], g_interval_160[i],
           g_interval_400[i], g_interval_1s[i]);
    last_count[i] = count[i];
  }
  fflush(stdout);
...
#endif
{
  ...
  while (!g_exit_flag) {
    ...
    while ((g_image_enc_queue[id].size() == MAX_QUEUE_FRAME_20) &&
           (!g_exit_flag)) {
#ifdef WIN32
      Sleep(1);
#else
...
#endif
{
  ...
  while (!g_exit_flag) {
    ...
    if (!writer.isOpened()) {
      writer.open("", VideoWriter::fourcc('H', '2', '6', '4'), 25,
                  Size(g_enc_cif_w, g_enc_cif_h),
                  "gop=30:gop_preset=2:bitrate=200k", true, g_card[id]);
      if (!writer.isOpened()) {
        return NULL;
      }
    }
...
#endif
{
  ...
  while (!g_exit_flag) {
    ...
    if (g_dump_flag[id]) {
      g_dump_flag[id] = 0;
#ifdef HAVE_BMCV
      imwrite("out-" + std::to_string(id) + "-" + std::to_string(pts) + ".jpg",
              frame);
#else
      imwrite("out-" + std::to_string(id) + "-" + std::to_string(pts) + ".jpg",
              image);
...
#else
      fps[id] = (double)count[id] * 1000.0 /
                ((tv1.tv_sec - tv2.tv_sec) * 1000.0 +
                 (tv1.tv_usec - tv2.tv_usec) / 1000.0);
#endif
    }
...
void usages(char **argv) {
#ifdef USING_SOC
  cout << "Usage: " << argv[0]
       << " thread_num input_video [enc_enable] input_video [enc_enable]..."
       << endl;
  cout << "enc enable: " << "0 disable encoder   1 enable encoder." << endl;
  cout << "For Examples: " << argv[0] << " 3 a.264 1 b.264 0 c.264 1" << endl;
  cout << "set env for display:"
       << "[export VIDMULTI_DISPLAY_FRAMERATE=0]   display only frames. "
       << endl;
  cout << "                    "
       << "[export VIDMULTI_DISPLAY_FRAMERATE=1]  display frame rate for each "
          "channel."
       << endl;
#else
  cout << "Usage: " << argv[0]
       << " thread_num input_video [card] [enc_enable] input_video [card] "
          "[enc_enable]..."
       << endl;
  cout << "enc enable: " << "0 disable encoder   1 enable encoder." << endl;
  cout << "For Examples: " << argv[0] << " 3 a.264 0 1 b.264 1 1 c.264 0 0"
       << endl;
  cout << "set env for display:"
       << "[export VIDMULTI_DISPLAY_FRAMERATE=0]   display only frames. "
       << endl;
  cout << "                    "
       ...
       << endl;
#endif
  cout << "set reopen model:" << "[export REOPEN_FLAG=0]   close reopen. "
       << endl;
  cout << "            " << "[export REOPEN_FLAG=1]  reopen every 400 frames."
       << endl;
}
...
int main(int argc, char **argv) {
  ...
  if (thread_num > MAX_THREAD_NUM) {
    cout << "The thread num is too big. The max thread num is "
         << MAX_THREAD_NUM << endl;
    return -1;
  }

  if (argc - 2 == thread_num) {  // only filename
    for (int i = 0; i < thread_num; i++) g_filename[i] = argv[2 + i];
  }
#ifdef USING_SOC
  ...
  /** Notice:
   * 1. In order to make sure signal_handler valid, video capture object should
   * not be processed in main thread. Otherwise crash of video capture will
   * cause main thread exit immediately, then signal_handler() has no chance to
   * release resource of video capture.
   * 2. register_signal_handler should be registered before pthread_create,
   * otherwise child thread will still to process signal in the signal_handler()
   */
  if (!test_mode)
...
#ifdef WIN32
    vc_thread[i] =
        CreateThread(NULL, 0, video_download_pthread, (void *)(&i), 0, NULL);
    int ret = 0;
#else
    int ret = pthread_create(&vc_thread[i], NULL, video_download_pthread,
                             thread_id + i);
#endif
    if (ret != 0) {
...
#ifdef WIN32
    image_thread[i] =
        CreateThread(NULL, 0, image_process_pthread, (void *)(&i), 0, NULL);
#else
    ret = pthread_create(&image_thread[i], NULL, image_process_pthread,
                         thread_id + i);
#endif
    if (ret != 0) {
      cout << "pthread create failed" << endl;
...
#ifdef WIN32
    enc_thread[i] =
        CreateThread(NULL, 0, video_encoder_pthread, (void *)(&i), 0, NULL);
#else
    ret = pthread_create(&enc_thread[i], NULL, video_encoder_pthread,
                         thread_id + i);
#endif
    if (ret != 0) {
      cout << "pthread create failed" << endl;
  ...
  while (cmd != 'q') {
    ...
    if (cmd == 'c')
      for (int i = 0; i < thread_num; i++) g_dump_flag[i] = 1;
  }
