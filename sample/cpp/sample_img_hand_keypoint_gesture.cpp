...
hand_keypoint_classification(
  ...
  for (size_t i = 0; i < out_hk.size(); i++) {
    ...
    float* data_buffer = reinterpret_cast<float *>(bin_data->getVirtualAddress()[0]);
...
int main(int argc, char **argv) {
  if (argc != 5) {
    printf("Usage: %s <hand_det_model_path> <hand_keypoint_model_path> \
 <hand_keypoint_cls_model_path> <image_path> \n", argv[0]);
    return -1;
  }
  std::string hd_model_path = argv[1];
  std::string hk_model_path = argv[2];
  std::string hc_model_path = argv[3];
  ...
  std::shared_ptr<BaseModel> model_hc = model_factory.getModel(
      ModelType::CLS_KEYPOINT_HAND_GESTURE, hc_model_path);
  if (!model_hc) {
  ...
  for (size_t i = 0; i < out_hk.size(); i++) {
    ...
    printf("hand[%d]: label: %d, score: %.2f\n", i, cls_meta->topk_class_ids[0], cls_meta->topk_scores[0]);

    char img_name[128];
    sprintf(img_name, "hand_keypoints_%d_label_%d.jpg", i, cls_meta->topk_class_ids[0]);
    visualize_keypoints_detection(hand_crops[i],obj_meta, 0.5 , std::string(img_name));
  }
