...
int skeleton[19][2] = {{15, 13}, {13, 11}, {16, 14}, {14, 12}, {11, 12}, {5, 11}, {6, 12},
                       ...
                       {0, 2},   {1, 3},   {2, 4},   {3, 5},   {4, 6}};
void visualize_keypoints_detection(std::shared_ptr<BaseImage> image, std::shared_ptr<ModelBoxLandmarkInfo> obj_meta, float score,
...
void visualize_object_detection(
  ...
  for (size_t i = 0; i < obj_meta->box_landmarks.size(); i++) {
    cv::rectangle(
        mat,
        cv::Rect(int(obj_meta->box_landmarks[i].x1), int(obj_meta->box_landmarks[i].y1),
    ...
    cv::putText(
        mat, sz_text,
        cv::Point(int(obj_meta->box_landmarks[i].x1), int(obj_meta->box_landmarks[i].y1)),
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<BaseModel> model_od = model_factory.getModel(
      TDL_MODEL_TYPE_KEYPOINT_DETECTION_YOLOV8, model_path);
  if (!model_od) {
  ...
  for (size_t i = 0; i < out_datas.size(); i++) {
    ...
    if (obj_meta->box_landmarks.size() == 0) {
      ...
      for (size_t j = 0; j < obj_meta->box_landmarks.size(); j++) {
        std::cout << "obj_meta_index: " << j << "  "
                  ...
                  << obj_meta->box_landmarks[j].y1 << " " << obj_meta->box_landmarks[j].x2
    ...
    visualize_keypoints_detection(image1,obj_meta, 0.5 ,"yolov8_keypoints.jpg");
  }
