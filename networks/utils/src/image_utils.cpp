...
void NormalizeToU8(cv::Mat *src_channels, const std::vector<float> &mean,
    ...
    for (size_t c = 0; c < 3; c++) {
        ...
        src_channels[c].convertTo(channels[2-c], CV_8SC1, scales[c], mean[c] * scales[c] + 0.5);
    }
}
...
void NormalizeAndSplitToU8(cv::Mat &prepared, const std::vector<float> &mean,
    ...
    cv::vpp::split(prepared, tmpchannels);
    for (size_t c = 0, b = 2; c < 3 && b>=0; c++, b--) {
        tmpchannels[c].convertTo(tmpchannels[c], CV_32F);
        tmpchannels[b].convertTo(channels[c], CV_8SC1, scales[c], mean[c] * scales[c] + 0.5);
    }
    #else
    ...
    if (prepared.channels() == 1) {
        ...
        int8_t *pB = (int8_t *)channels[0].data;

        float mG = mean[1] * scales[1] + NEON_ROUNDING_VALUE;
        float mB = mean[0] * scales[0] + NEON_ROUNDING_VALUE;

        float32x4_t sumG = vdupq_n_f32(mG);
        float32x4_t mulG = vdupq_n_f32(scales[1]);

        float32x4_t sumB = vdupq_n_f32(mB);
        float32x4_t mulB = vdupq_n_f32(scales[0]);
        ...
        for (i = 0; i < i_limit;) {
            ...
            GENERATE_NEON_SPLITBYCHANNEL(B, src.val[0], sumB, mulB, pB);
        ...
        for (; i < total_pixel; i++) {
            *pG = cv::saturate_cast<char>((float)pSrc[1] * scales[1] + mG);
            *pB = cv::saturate_cast<char>((float)pSrc[0] * scales[0] + mB);
            pG++;
            pB++;
            pSrc += 2;
        }
        ...
        unsigned char *pSrc = (unsigned char *)prepared.data;
        // Original OpenCV in BGR
        int8_t *pR = (int8_t *)channels[2].data;
        int8_t *pG = (int8_t *)channels[1].data;
        int8_t *pB = (int8_t *)channels[0].data;

        float mR = mean[2] * scales[2] + NEON_ROUNDING_VALUE;
        float mG = mean[1] * scales[1] + NEON_ROUNDING_VALUE;
        float mB = mean[0] * scales[0] + NEON_ROUNDING_VALUE;

        float32x4_t sumR = vdupq_n_f32(mR);
        float32x4_t mulR = vdupq_n_f32(scales[2]);
        ...
        for (i = 0; i < i_limit;) {
            ...
            GENERATE_NEON_SPLITBYCHANNEL(R, src.val[2], sumR, mulR, pR);
            GENERATE_NEON_SPLITBYCHANNEL(G, src.val[1], sumG, mulG, pG);
            GENERATE_NEON_SPLITBYCHANNEL(B, src.val[0], sumB, mulB, pB);
        ...
        // Original OpenCV in BGR
        for (; i < total_pixel; i++) {
            *pR = cv::saturate_cast<char>((float)pSrc[2] * scales[2] + mR);
            *pG = cv::saturate_cast<char>((float)pSrc[1] * scales[1] + mG);
            *pB = cv::saturate_cast<char>((float)pSrc[0] * scales[0] + mB);
            pR++;
            pG++;
        ...
        unsigned char *pSrc = (unsigned char *)prepared.data;
        // Original OpenCV in BGR
        int8_t *pA = (int8_t *)channels[3].data;
        int8_t *pR = (int8_t *)channels[2].data;
        int8_t *pG = (int8_t *)channels[1].data;
        int8_t *pB = (int8_t *)channels[0].data;

        float mA = mean[3] * scales[3] + NEON_ROUNDING_VALUE;
        float mR = mean[2] * scales[2] + NEON_ROUNDING_VALUE;
        float mG = mean[1] * scales[1] + NEON_ROUNDING_VALUE;
        float mB = mean[0] * scales[0] + NEON_ROUNDING_VALUE;

        float32x4_t sumA = vdupq_n_f32(mA);
        float32x4_t mulA = vdupq_n_f32(scales[3]);
        ...
        for (i = 0; i < i_limit;) {
            ...
            GENERATE_NEON_SPLITBYCHANNEL(A, src.val[3], sumA, mulA, pA);
            GENERATE_NEON_SPLITBYCHANNEL(R, src.val[2], sumR, mulR, pR);
            GENERATE_NEON_SPLITBYCHANNEL(G, src.val[1], sumG, mulG, pG);
            GENERATE_NEON_SPLITBYCHANNEL(B, src.val[0], sumB, mulB, pB);
        ...
        // Original OpenCV in BGR
        for (; i < total_pixel; i++) {
            *pA = cv::saturate_cast<char>((float)pSrc[2] * scales[3] + mA);
            *pR = cv::saturate_cast<char>((float)pSrc[2] * scales[2] + mR);
            *pG = cv::saturate_cast<char>((float)pSrc[1] * scales[1] + mG);
            *pB = cv::saturate_cast<char>((float)pSrc[0] * scales[0] + mB);
            pA++;
...
// Caution! Neon version and OpenCV version has a little difference cause the rounding issue.
void NormalizeAndSplitToU8(cv::Mat &prepared, vector<cv::Mat> &channels, float MeanR, float alphaR,
                           float MeanG, float alphaG, float MeanB, float alphaB, int width,
                           int height) {
#ifndef __ARM_ARCH
    vector<cv::Mat> tmpchannels;
    if (prepared.type() != CV_32FC3) prepared.convertTo(prepared, CV_32FC3);
    cv::split(prepared, tmpchannels);

    tmpchannels[0].convertTo(channels[0], CV_8SC1, alphaB, MeanB * alphaB + 0.5);
    tmpchannels[1].convertTo(channels[1], CV_8SC1, alphaG, MeanG * alphaG + 0.5);
    tmpchannels[2].convertTo(channels[2], CV_8SC1, alphaR, MeanR * alphaR + 0.5);
#else
    unsigned char *pSrc = (unsigned char *)prepared.data;
    // Original OpenCV in BGR
    int8_t *pR = (int8_t *)channels[2].data;
    ...
    float mR = MeanR * alphaR + NEON_ROUNDING_VALUE;
    float mG = MeanG * alphaG + NEON_ROUNDING_VALUE;
    float mB = MeanB * alphaB + NEON_ROUNDING_VALUE;
    ...
    for (i = 0; i < i_limit;) {
        ...
        uint16x8_t Ru16 = vmovl_u8(src.val[2]);
        uint16x8_t Gu16 = vmovl_u8(src.val[1]);
        uint16x8_t Bu16 = vmovl_u8(src.val[0]);
        ...
        int32x4_t Rl = vcvtq_s32_f32_r(R0);
        int32x4_t Rh = vcvtq_s32_f32_r(R1);

        int32x4_t Gl = vcvtq_s32_f32_r(G0);
        int32x4_t Gh = vcvtq_s32_f32_r(G1);

        int32x4_t Bl = vcvtq_s32_f32_r(B0);
        int32x4_t Bh = vcvtq_s32_f32_r(B1);
        // OpenCV saturates values, so use vqmovn_s32 instead of vmovn_s32
        int16x8_t Rs = vcombine_s16(vqmovn_s32(Rl), vqmovn_s32(Rh));
        int16x8_t Gs = vcombine_s16(vqmovn_s32(Gl), vqmovn_s32(Gh));
        int16x8_t Bs = vcombine_s16(vqmovn_s32(Bl), vqmovn_s32(Bh));

        vst1_s8(pR, vqmovn_s16(Rs));
        vst1_s8(pG, vqmovn_s16(Gs));
        vst1_s8(pB, vqmovn_s16(Bs));
    ...
    // Original OpenCV in BGR
    for (; i < total_pixel; i++) {
        *pR = cv::saturate_cast<char>((float)pSrc[2] * alphaR + mR);
        *pG = cv::saturate_cast<char>((float)pSrc[1] * alphaG + mG);
        *pB = cv::saturate_cast<char>((float)pSrc[0] * alphaB + mB);
        pR++;
        pG++;
        pB++;
        pSrc += 3;
