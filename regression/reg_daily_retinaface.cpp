...
TEST_F(RetinafaceTestSuite, get_vpss_config) {
  ...
  for (size_t test_index = 0; test_index < m_json_object.size(); test_index++) {
    ...
    EXPECT_EQ(CVI_TDL_GetVpssChnConfig(m_tdl_handle, CVI_TDL_SUPPORTED_MODEL_RETINAFACE, 342, 608,
                                       0, &vpssconfig),
              CVI_TDL_SUCCESS);
...
TEST_F(RetinafaceTestSuite, skip_vpss_preprocess) {
  for (size_t test_index = 0; test_index < m_json_object.size(); test_index++) {
    ...
    {
      TDLModelHandler tdlmodel(m_tdl_handle, CVI_TDL_SUPPORTED_MODEL_RETINAFACE,
    ...
    }
    {
      ...
      EXPECT_EQ(CVI_TDL_RetinaFace(m_tdl_handle, frame.getFrame(), face_meta),
