...
class Hardhat_DetectionTestSuite : public CVI_TDLModelTestSuite {
 public:
  Hardhat_DetectionTestSuite() : CVI_TDLModelTestSuite("daily_reg_HatD.json", "reg_daily_hatd") {}
...
TEST_F(Hardhat_DetectionTestSuite, open_close_model) {
  ASSERT_EQ(CVI_TDL_OpenModel(m_tdl_handle, CVI_TDL_SUPPORTED_MODEL_HEAD_PERSON_DETECTION,
                              m_model_path.c_str()),
            CVI_TDL_SUCCESS)
      << "failed to set model path: " << m_model_path.c_str();

  const char *model_path_get =
      CVI_TDL_GetModelPath(m_tdl_handle, CVI_TDL_SUPPORTED_MODEL_HEAD_PERSON_DETECTION);

  EXPECT_PRED2([](auto s1, auto s2) { return s1 == s2; }, m_model_path,
               std::string(model_path_get));

  ASSERT_EQ(CVI_TDL_CloseModel(m_tdl_handle, CVI_TDL_SUPPORTED_MODEL_HEAD_PERSON_DETECTION),
            CVI_TDL_SUCCESS);
}
...
TEST_F(Hardhat_DetectionTestSuite, accuracy) {
  ...
  for (nlohmann::json::iterator iter = results.begin(); iter != results.end(); iter++) {
    ...
    TDLObject<cvtdl_face_t> vehicle_meta;
    init_face_meta(vehicle_meta, 1);
    ASSERT_EQ(CVI_TDL_RetinaFace_Hardhat(m_tdl_handle, vframe, vehicle_meta), CVI_TDL_SUCCESS);
    printf("boxes===================================\n");
