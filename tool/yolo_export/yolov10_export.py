...
def get_args():
    ...
    parser.add_argument('--weights', type=str, default='./yolov10n.pt', help='weights path')
...
def forward2(self, x):
    # print(self.nl)
    x_reg = [self.one2one_cv2[i](x[i]) for i in range(self.nl)]
    x_cls = [self.one2one_cv3[i](x[i]) for i in range(self.nl)]
    return x_reg + x_cls
