...
/**
 * @brief Get object detection model's ID and inference function according to
 * model_name.
 * @param model_name [in] model name
 * @param model_id [out] model id
 * @param inference_func [out] inference function in TDL SDK
 * @return int Return CVI_TDL_SUCCESS if corresponding model information is
 * found. Otherwise, return CVI_TDL_FAILURE
 */
CVI_S32 get_od_model_info(const char *model_name, CVI_TDL_SUPPORTED_MODEL_E *model_id,
                          ODInferenceFunc *inference_func);

/**
 * @brief Get person detection model's ID and inference function according to
 * model_name.
 * @param model_name [in] model name
 * @param model_id [out] model id
 * @param inference_func [out] inference function in TDL SDK
