...
// 复制数据：对每一帧的每个通道，先将数据类型转换为 float，再使用 memcpy 写入目标数组  
// 注意: 每个 cv::Mat 数据默认类型为 unsigned char，因此需要先转换类型
// 如果通道数为3，则调整通道顺序从 BGR 转换为 RGB
if (channels == 3) {
  ...
  for (int f = 0; f < frame_count; ++f) {
    for (int c = 0; c < channels; ++c) {
      cv::Mat floatMat;
      // 使用映射后的通道，将 cv::Mat 从 unsigned char 转换为 float，不进行归一化
      ...
      float *dst = ptr + f * (channels * height * width) + c * (height * width);
