...
LstrLane::LstrLane() {
  net_param_.pre_params.scale[0] = 0.014598;
  net_param_.pre_params.scale[1] = 0.0150078;
  net_param_.pre_params.scale[2] = 0.0142201;
...
int32_t LstrLane::outputParse(
  ...
  for (uint32_t b = 0; b < (uint32_t)input_tensor.shape[0]; b++) {
    ...
    for (int i = 0; i < final_index.size(); i++) {
      ...
      lane_landmark.landmarks_y.push_back(DETECTION_UPPER_LIMIT * obj->image_height);
      lane_landmark.landmarks_x.push_back((true_x1 + (DETECTION_UPPER_LIMIT - true_y1) * (true_x2 - true_x1) / (true_y2 - true_y1)) * obj->image_width);
      lane_landmark.landmarks_y.push_back(DETECTION_LOWER_LIMIT * obj->image_height);
      lane_landmark.landmarks_x.push_back((true_x1 + (DETECTION_LOWER_LIMIT - true_y1) * (true_x2 - true_x1) / (true_y2 - true_y1)) * obj->image_width);
