...
void Yolo26Detection::getTopkIndex(const std::vector<float> &scores,
  ...
  // 对齐 inferonnx_yolo26_batch_v2.py: np_get_topk_index 的两阶段 top-k 逻辑：
  ...
  // Python 脚本里 k = max_det（export 模式）；但为避免 k > anchors
  // 导致行为/越界问题， 这里保持安全：k = min(max_det, num_anchors)。
