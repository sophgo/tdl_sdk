...
void quant_feat(float *p_feat_src, float q_scale, int len, float logmin_val,
                int8_t *p_feat_quant) {
  for (int i = 0; i < len; i++) {
...
MelFeatureExtract::MelFeatureExtract(int num_frames, int sr, int n_fft,
                                     int n_hop, int n_mel, int fmin, int fmax,
                                     const std::string &mode, bool htk,
...
void MelFeatureExtract::pad(Vectorf &x, int left, int right,
...
void MelFeatureExtract::melspectrogram_optimze(short *p_data, int data_len,
                                               int8_t *p_dst, int dst_len,
                                               float q_scale, bool fix,
                                               float eps, float s, float alpha,
                                               float delta, float r) {
  ...
  for (int i = 0; i < n_frames; ++i) {
    ...
    for (int j = 0; j < num_fft_; j++) {
      ...
      segment[j] =
          p_data[srcidx] * scale;  // TODO:fuquan.ke this could be optimized
    }
    ...
    if (fix) {  // use pcen
      ...
      melspec::Vectorf pcen_data =
          (rowv.array() / (last_state.array() + eps).pow(alpha) + delta)
              .pow(r) -
          pow(delta, r);
...
std::map<float, int> MelFeatureExtract::generate_seg_pack_idx(int pack_idx,
                                                              int pack_len,
                                                              int hop_len,
                                                              int wav_len) {
...
int MelFeatureExtract::melspectrogram_pack_optimize(
    short *p_data, int data_len, int pack_len, int start_pack_idx,
    int8_t *p_dst, int dst_len, float q_scale, bool fix, float eps, float s,
    ...
    if (seg_map.count(i)) {
      if (fix) {
        Eigen::Map<Eigen::Matrix<float, 1, Eigen::Dynamic, Eigen::RowMajor>>
            rowv(mp_sft_mag_vec_ + i * num_mel_, 1, num_mel_);
        last_state = (1 - s) * last_state + s * rowv;
        melspec::Vectorf pcen_data =
            (rowv.array() / (last_state.array() + eps).pow(alpha) + delta)
                .pow(r) -
