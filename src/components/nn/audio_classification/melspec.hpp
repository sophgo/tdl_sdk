...
class MelFeatureExtract {
 public:
  MelFeatureExtract(int num_frames, int sr, int n_fft, int n_hop, int n_mel,
                    int fmin, int fmax, const std::string &mode, bool htk,
                    bool center = true, int power = 2, bool is_log = true);
  ~MelFeatureExtract();
  ...
  void melspectrogram_optimze(short *p_data, int data_len, int8_t *p_dst,
                              int dst_len, float q_scale, bool fixed = false,
                              float eps = 1E-6, float s = 0.025,
                              float alpha = 0.98, float delta = 2,
                              float r = 0.5);
  void pad(Vectorf &x, int left, int right, const std::string &mode,
           float value);
   ...
   */
  std::map<float, int> generate_seg_pack_idx(int pack_idx, int pack_len,
   ...
   */
  int melspectrogram_pack_optimize(short *p_data, int data_len, int pack_len,
                                   int start_pack_idx, int8_t *p_dst,
                                   int dst_len, float q_scale,
                                   bool fixed = false, float eps = 1E-6,
                                   float s = 0.025, float alpha = 0.98,
