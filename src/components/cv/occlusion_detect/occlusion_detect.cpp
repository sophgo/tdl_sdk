...
int OcclusionDetector::detect(std::shared_ptr<BaseImage> image, cvtdl_occlusion_meta_t* meta) {
    if (!image || !meta) {
    ...
    int max_area = (num_labels > 1) ? *std::max_element(sizes.begin() + 1, sizes.end()) : 0;
    ...
    if ((int)occlusionStates_.size() > meta->sensitive_th) {
        ...
        int occludedCount = std::accumulate(occlusionStates_.begin(), occlusionStates_.end(), 0);
...
void OcclusionDetector::reset() {
    occlusionStates_.clear();
}
