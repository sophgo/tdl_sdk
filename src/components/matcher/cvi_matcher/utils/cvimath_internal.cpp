...
static MatrixLmem matrix_lmem[5];
#define DEBUG (0)
#define DBG(fmt, ...)                             \
  do {                                            \
...
MatrixLmem *bmk1880v2_matrix_lmem_prealloc_align(KernelContext *ctx,
                                                 MatrixLmem *pre, uint32_t la,
                                                 MatrixLmemShape s,
  ...
  MatrixLmem *t = nullptr;
  if (pre) {
    t = pre;
  } else {
    // 使用 C++ 风格的类型转换和内存分配
    t = static_cast<MatrixLmem *>(malloc(sizeof(MatrixLmem)));
  }
  ...
  t->stride.h = s.w * val;
  if (eu_align)
    t->stride.c = alignUp(s.w * val, eu_num);
  else
    t->stride.c = s.w * val;
  t->stride.n = t->stride.c * ceilingFunc(s.c, npu_num);

  uint32_t needed = alignUp(t->shape.n * t->stride.n, eu_num);
...
void bmk1880v2_lmem_free_prealloc_bf16_matrix(KernelContext *ctx,
                                              bool is_pre_alloc,
                                              const MatrixLmem *t) {
  ...
  if (!is_pre_alloc) {
    free(const_cast<MatrixLmem *>(t));  // 使用 const_cast 消除 const 限定符
  }
}

static void tdma_store_stride_bf16(KernelContext *ctx, MatrixLmem *tlp,
                                   uint64_t ga_dst,
                                   const MatrixTGmemStride &ts_stride,
                                   ctrl_t ctrl) {
  ...
  TdmaL2gMatrixCopyParam p1;
  p1.src = tlp;
  p1.dst = &ts_data;
  ctx->ops->tdma_l2g_bf16_matrix_copy(ctx, &p1);
}

static void tdma_load_stride_bf16(KernelContext *ctx, MatrixLmem *tlp,
                                  uint64_t ga_src,
                                  const MatrixTGmemStride &ts_stride,
                                  ctrl_t ctrl) {
  ASSERT(tlp != nullptr);
...
//       B opd2(1, 1, 1, 16)
//
static void matrix_multiplication(KernelContext *ctx,
                                  MatrixMultiplicationParam *p) {
  ...
  tl_res.stride = ctx->ops->ml_default_stride(ctx, tl_res.shape, CVK_FMT_BF16,
                                              /*eu_align=*/1);
  ...
  tl_right.stride = ctx->ops->ml_default_stride(ctx, tl_right.shape,
                                                CVK_FMT_BF16, /*eu_align=*/1);

  MatrixLmem tl_bias = {0};
  if (p->bias) {
    ...
    tl_bias.stride = ctx->ops->ml_default_stride(ctx, tl_bias.shape,
                                                 CVK_FMT_BF16, /*eu_align=*/1);
  }

  MatrixMultiplicationParam p2;
  // copy p to p2
  p2.res = p->res;
  p2.left = p->left;
  p2.right = p->right;
...
static void fc_slicing_multi_dimention(
    KernelContext *ctx, uint32_t layer_id, gaddr_t global_offset_bottom_data,
    gaddr_t global_offset_weight_data, gaddr_t global_offset_bias_data,
    gaddr_t global_offset_top_data, int input_row_num, int input_col_num,
    int weight_col_num, int have_bias, int do_activation,
  ...
  // Tiled Y
  MatrixLmem tl_tiled_Y = {0};
  tl_tiled_Y.fmt = CVK_FMT_BF16;

  // Tiled L
  MatrixLmem tl_tiled_L = {0};
  tl_tiled_L.fmt = CVK_FMT_BF16;

  // Tiled R
  MatrixLmem tl_tiled_R = {0};
  tl_tiled_R.fmt = CVK_FMT_BF16;

  // Tiled B
  MatrixLmem tl_tiled_B = {0};
  if (have_bias) {
    ...
    tl_tiled_B.shape = ctx->ops->ml_default_shape(
        ctx, sizeof(uint32_t) / sizeof(uint16_t), tiled_N,
        CVK_FMT_BF16);  // 2 x 16bit
    tl_tiled_B.stride = ctx->ops->ml_default_stride(
        ctx, tl_tiled_B.shape, CVK_FMT_BF16, /*eu_align=*/1);
  }
  ...
  do {
    ...
    tl_tiled_Y.shape =
        ctx->ops->ml_default_shape(ctx, M, tiled_N, CVK_FMT_BF16);
    tl_tiled_Y.stride = ctx->ops->ml_default_stride(
        ctx, tl_tiled_Y.shape, CVK_FMT_BF16, /*eu_align=*/1);
    required_size +=
        ctx->ops->lmem_ps32_matrix_to_size(ctx, tl_tiled_Y.shape, CVK_FMT_BF16,
                                           /*eu_align=*/1);

    // tiled L, 16bit
    tl_tiled_L.start_address = required_size;
    tl_tiled_L.shape =
        ctx->ops->ml_default_shape(ctx, M, tiled_K, CVK_FMT_BF16);
    tl_tiled_L.stride = ctx->ops->ml_default_stride(
        ctx, tl_tiled_L.shape, CVK_FMT_BF16, /*eu_align=*/1);
    required_size += ctx->ops->lmem_matrix_to_size(
        ctx, tl_tiled_L.shape, CVK_FMT_BF16, /*eu_align=*/1);

    // tiled R, 16bit
    tl_tiled_R.start_address = required_size;
    tl_tiled_R.shape =
        ctx->ops->ml_default_shape(ctx, tiled_K, tiled_N, CVK_FMT_BF16);
    tl_tiled_R.stride = ctx->ops->ml_default_stride(
        ctx, tl_tiled_R.shape, CVK_FMT_BF16, /*eu_align=*/1);
    required_size += ctx->ops->lmem_matrix_to_size(
        ctx, tl_tiled_R.shape, CVK_FMT_BF16, /*eu_aligned=*/1);
    ...
    if (required_size <= lane_size) {
      // LLVM_DEBUG(printf("  tiled_Y %d, tiled_L %d, tiled_R %d, tiled_B %d,
      // required_size %d\n",
      //                              ctx->ops->lmem_ps32_matrix_to_size(ctx,
      //                              tl_tiled_Y.shape, CVK_FMT_BF16,
      //                              /*eu_align=*/1),
      //                              ctx->ops->lmem_matrix_to_size(ctx,
      //                              tl_tiled_L.shape, CVK_FMT_BF16,
      //                              /*eu_align=*/1),
      //                              ctx->ops->lmem_matrix_to_size(ctx,
      //                              tl_tiled_R.shape, CVK_FMT_BF16,
      //                              /*eu_align=*/1),
      //                              ctx->ops->lmem_matrix_to_size(ctx,
      //                              tl_tiled_B.shape, CVK_FMT_BF16,
      //                              /*eu_align=*/1), required_size););
  ...
  LLVM_DEBUG(printf("  tiled_Y(%d, %d) = tiled_L(%d, %d) * tiled_R(%d, %d) + "
                    "tiled_B(%d, %d),"
                    " required_size %d kB\n",
  ...
  LLVM_DEBUG(
      printf("  tiled_Y shape (n=%d, c=%d, w=%d, col=%d), stride(n=%d, c=%d, "
             "h=%d)\n"
             "  tiled_L shape (n=%d, c=%d, w=%d, col=%d), stride(n=%d, c=%d, "
             "h=%d)\n"
             "  tiled_R shape (n=%d, c=%d, w=%d, col=%d), stride(n=%d, c=%d, "
             "h=%d)\n"
             "  tiled_B shape (n=%d, c=%d, w=%d, col=%d), stride(n=%d, c=%d, "
             "h=%d)\n",
  ...
  for (uint32_t offset_N = 0; offset_N < N; offset_N += tiled_N) {
    ...
    for (uint32_t offset_K = 0; offset_K < K; offset_K += tiled_K) {
      ...
      // tiled Y, 32bit
      tl_tiled_Y.start_address = required_size;
      tl_tiled_Y.shape = ctx->ops->ml_default_shape(
          ctx, M, width_N, CVK_FMT_BF16);  // actual width
      required_size += ctx->ops->lmem_ps32_matrix_to_size(ctx, tl_tiled_Y.shape,
                                                          CVK_FMT_BF16,
                                                          /*eu_align=*/1);

      // Load tiled L from global memory, input
      tl_tiled_L.start_address = required_size;
      tl_tiled_L.shape = ctx->ops->ml_default_shape(
          ctx, M, width_K, CVK_FMT_BF16);  // actual width
      tl_tiled_L.stride =
          ctx->ops->ml_default_stride(ctx, tl_tiled_L.shape, CVK_FMT_BF16,
                                      /*eu_align=*/1);
      required_size += ctx->ops->lmem_matrix_to_size(
          ctx, tl_tiled_L.shape, CVK_FMT_BF16, /*eu_align=*/1);
      MatrixTGmemStride ts_stride;
      ...
      tl_tiled_R.start_address = required_size;
      tl_tiled_R.shape = ctx->ops->ml_default_shape(
          ctx, width_K, width_N, CVK_FMT_BF16);  // actual width
      tl_tiled_R.stride =
          ctx->ops->ml_default_stride(ctx, tl_tiled_R.shape, CVK_FMT_BF16,
                                      /*eu_align=*/1);
      required_size += ctx->ops->lmem_matrix_to_size(
          ctx, tl_tiled_R.shape, CVK_FMT_BF16, /*eu_aligned=*/1);

      ts_stride.row = N * sizeof(uint16_t);
      tdma_load_stride_bf16(ctx, &tl_tiled_R,
                            global_offset_weight_data +
                                (offset_K * N + offset_N) * sizeof(uint16_t),
                            ts_stride,  // original column width
                            CTRL_NEURON);
      ...
      if (B_needed) {
        ...
        tl_tiled_B.shape = ctx->ops->ml_default_shape(
            ctx, sizeof(uint32_t) / sizeof(uint16_t), width_N,
            CVK_FMT_BF16);  // 2 x 16bit, actual width
        tl_tiled_B.stride = ctx->ops->ml_default_stride(
            ctx, tl_tiled_B.shape, CVK_FMT_BF16, /*eu_align=*/1);
        required_size +=
            ctx->ops->lmem_matrix_to_size(ctx, tl_tiled_B.shape, CVK_FMT_BF16,
                                          /*eu_aligned=*/1);
      ...
      if ((ps32_mode == 0 || ps32_mode == 1) && do_activation &&
...

      {
        ...
        LLVM_DEBUG(printf("  [offset_N=%d][offset_K=%d] L(%d, %d), R(%d, %d)\n",
                          offset_N, offset_K, p.left->shape.n,
...
void cvm_fc_forward_kernel(KernelContext *ctx, uint32_t layer_id,
                           gaddr_t bottom_data_gaddr, gaddr_t weight_data_gaddr,
                           gaddr_t bias_data_gaddr, gaddr_t top_data_gaddr,
                           int in_row, int in_col, int out_col, int have_bias,
                           int do_activation, int activation_method) {
  // LLVM_DEBUG(
  //    printf("bf16_fc_forward_kernel\n"
  //           "    bottom_gaddr 0x%lx, weight_gaddr 0x%lx, bias_gaddr 0x%lx,
  //           top_gaddr 0x%lx\n" "    in (%d, %d), out (%d)\n" "    has_bias
  //           %d, do_activation %d, activation_method %d\n", bottom_data_gaddr,
  //           weight_data_gaddr, bias_data_gaddr, top_data_gaddr, in_row,
  //           in_col, out_col, have_bias, do_activation, activation_method););

  fc_slicing_multi_dimention(ctx, layer_id, bottom_data_gaddr,
                             weight_data_gaddr, bias_data_gaddr, top_data_gaddr,
                             in_row, in_col, out_col, have_bias, do_activation,
                             activation_method);
}

// gemm
inline static size_t get_neuron_csize_local(KernelContext *ctx, size_t h,
                                            size_t w, KernelFmt fmt) {
  size_t size = h * w * bitsizeOfFmt(fmt) / 8;
  // ctx->info.eu_num neurons align
  return ALIGN(size, ctx->info.eu_num);
}

static int get_fmt_byte_sz(KernelFmt fmt) { return bitsizeOfFmt(fmt) / 8; }

static uint64_t get_slice_global_offset(uint64_t global_offset,
                                        size_t row_slice_idx,
                                        size_t col_slice_idx, size_t row_num,
                                        size_t col_num, size_t row_slice_num,
                                        size_t col_slice_num, KernelFmt fmt) {
  ...
  if (row_slice_idx < (row_num % row_slice_num)) {
    ...
    slice_offset_row =
        (row_num % row_slice_num) * (row_num / row_slice_num + 1) +
  ...
  if (col_slice_idx < (col_num % col_slice_num)) {
    ...
    slice_offset_col =
        (col_num % col_slice_num) * (col_num / col_slice_num + 1) +
...
#define NPU_SHIFT (getNumShift(ctx->info.npu_num))
#define MAX(a, b) (((a) > (b)) ? (a) : (b))
// #define SMALL_TEST (1)
static size_t get_slice_num(KernelContext *ctx, size_t M, size_t N, size_t K,
                            size_t *slice_num, KernelFmt fmt) {
  int fmt_byte_sz = get_fmt_byte_sz(fmt);
#ifdef SMALL_TEST
  ...
  size_t size_A = M * ceilingFuncShift(C_param, NPU_SHIFT) * csize_local;
  C_param = (N + W_param - 1) / W_param;
  size_t size_B = K * ceilingFuncShift(C_param, NPU_SHIFT) * csize_local;
  int res_byte_sz = 1;
  if (fmt != CVK_FMT_BF16) {
  ...
  size_t size_C =
      res_byte_sz * M * ceilingFuncShift(C_param, NPU_SHIFT) * csize_local;
  ...
  if (size_A <= bank_size && size_B <= bank_size && size_C <= bank_size) {
    ...
    slice_num[0] =
        MAX(ceilingFunc(size_A, bank_size), ceilingFunc(size_C, bank_size));
    // split C local memory size
    size_t slice_size = ceilingFunc(M, slice_num[0]);
    C_param = (N + ctx->info.eu_num - 1) / ctx->info.eu_num;
    size_t slice_mem_C =
        slice_size * ceilingFuncShift(C_param, NPU_SHIFT) * csize_local;
    if (slice_mem_C > bank_size) return 3;
    return 1;
  } else if (size_A <= bank_size) {
    slice_num[1] =
        MAX(ceilingFunc(size_B, bank_size), ceilingFunc(size_C, bank_size));
    ...
    if (slice_num[1] > 1) {
      ...
      slice_num[1] = ceilingFunc(N, N_floor);
    }
    //
    if (ceilingFunc(ceilingFunc(N, ctx->info.eu_num), NPU_SHIFT) <
        (int)slice_num[1])
      return 3;
    return 2;
  } else {
...
static int load_matrix(KernelContext *ctx, MatrixLmem *tlp, uint64_t gaddr_in,
                       MatrixGmemShape shape, MatrixTGmemStride stride,
                       KernelFmt fmt) {
  // load matrix data
  MatrixGmem ts_data;
...
static int store_matrix(KernelContext *ctx, MatrixLmem *tlp, uint64_t gaddr_in,
                        MatrixGmemShape shape, MatrixTGmemStride stride,
                        KernelFmt fmt) {
  MatrixGmem ts_data;
  ts_data.base_reg_index = 0;
...
static void _matrix_multiplication(KernelContext *ctx, MatrixLmem *tlp_a,
                                   MatrixLmem *tlp_b, MatrixLmem *tlp_c,
                                   int ps32_mode) {
  // mac
  MatrixMultiplicationParam p2 = {};
  p2.bias = nullptr;
...
static void strategy_no_slice(KernelContext *ctx, size_t M, size_t N, size_t K,
                              uint64_t gaddr_a, uint64_t gaddr_b,
                              uint64_t gaddr_c, KernelFmt fmt) {
  MatrixLmem *tlp_a;
  MatrixLmem *tlp_b;
  MatrixLmem *tlp_c;
  int fmt_byte_sz = get_fmt_byte_sz(fmt);
  ...
  MatrixLmemShape shape_a = ctx->ops->ml_default_shape(ctx, M, K, fmt);
  MatrixLmemShape shape_b = ctx->ops->ml_default_shape(ctx, K, N, fmt);
  MatrixLmemShape shape_c = ctx->ops->ml_default_shape(ctx, M, N, fmt);
...
static void strategy_slice_on_M(KernelContext *ctx, size_t M, size_t N,
                                size_t K, uint64_t gaddr_a, uint64_t gaddr_b,
                                uint64_t gaddr_c, size_t slice_num,
                                KernelFmt fmt) {
  MatrixLmem *tlp_b;
  int fmt_byte_sz = get_fmt_byte_sz(fmt);

  MatrixLmemShape s_B = ctx->ops->ml_default_shape(ctx, K, N, fmt);
  ...
  for (size_t slice_idx = 0; slice_idx < slice_num; slice_idx++) {
    ...
    MatrixLmemShape s_A = ctx->ops->ml_default_shape(ctx, M_slice, K, fmt);
    ...
    MatrixLmemShape s_C = ctx->ops->ml_default_shape(ctx, M_slice, N, fmt);
    MatrixLmem *tlp_c;
    ...
    if (fmt != CVK_FMT_BF16) {
      ...
      C_slice_global_offset =
          get_slice_global_offset(gaddr_c, 0, 0, M, N, slice_num, 1, fmt);
...
static void strategy_slice_on_N(KernelContext *ctx, size_t M, size_t N,
                                size_t K, uint64_t gaddr_a, uint64_t gaddr_b,
                                uint64_t gaddr_c, size_t slice_num,
                                KernelFmt fmt) {
  MatrixLmem *tlp_a;
  ...
  MatrixLmemShape s_a = ctx->ops->ml_default_shape(ctx, M, K, fmt);
  tlp_a = ctx->ops->lmem_alloc_matrix(ctx, s_a, fmt, CTRL_AL);
  ...
  for (size_t slice_idx = 0; slice_idx < slice_num; slice_idx++) {
    ...
    MatrixLmemShape s_b = ctx->ops->ml_default_shape(ctx, K, N_slice, fmt);
    MatrixLmem *tlp_b;
    tlp_b = ctx->ops->lmem_alloc_matrix(ctx, s_b, fmt, CTRL_AL);
    ...
    // c for answer
    MatrixLmemShape s_c = ctx->ops->ml_default_shape(ctx, M, N_slice, fmt);
    MatrixLmem *tlp_c;
    if (fmt == CVK_FMT_BF16) {
      tlp_c = ctx->ops->lmem_alloc_matrix(ctx, s_c, fmt, CTRL_AL);
    } else {
    ...
    if (fmt != CVK_FMT_BF16) {
      // int32 pack
      C_slice_global_offset =
          get_slice_global_offset(gaddr_c, 0, 0, M, N, 1, slice_num, fmt);
      C_slice_global_offset += pack_shift;
...
static void slice_split_strategy(KernelContext *ctx, size_t M, size_t N,
                                 size_t K, size_t *slice_num, KernelFmt fmt) {
  int fmt_byte_sz = get_fmt_byte_sz(fmt);
  size_t W_param = ctx->info.eu_num;
  ...
  size_t C_param = (K + W_param - 1) / W_param;
  size_t local_size_A =
      M * (ceilingFuncShift(C_param, NPU_SHIFT)) * channel_size_local;
  size_t slice_num_A = (local_size_A + bank_size_half - 1) / (bank_size_half);
  size_t col_slice_time_A = ceilingFuncShift(C_param, NPU_SHIFT);
  size_t row_slice_time_A = (slice_num_A < M) ? slice_num_A : M;

  // weight blob
  C_param = (N + W_param - 1) / W_param;
  size_t local_size_B =
      K * (ceilingFuncShift(C_param, NPU_SHIFT)) * channel_size_local;
  ...
  size_t local_size_C =
      (M + 1) * (ceilingFuncShift(C_param, NPU_SHIFT)) * channel_size_local;
  size_t slice_num_C =
      (local_size_C + bank_size * outputs_nr - 1) / (bank_size * outputs_nr);
  size_t col_slice_time_C = ceilingFuncShift(C_param, NPU_SHIFT);
  ...
  if (slice_num_B > slice_num[1]) {
    ...
    slice_num[2] = get_max(slice_num[2],
                           (row_slice_time_B > tmp) ? tmp : row_slice_time_B);
  }
  // fine-tuning
  ...
  while (true) {
    ...
    size_t local_size_B = matrix_shape[1] *
                          (ceilingFuncShift(C_param_weight_col, NPU_SHIFT)) *
                          channel_size_local;
    size_t local_size_C = matrix_shape[0] *
                          (ceilingFuncShift(C_param_weight_col, NPU_SHIFT)) *
                          channel_size_local;
    size_t local_size_A = matrix_shape[0] *
                          (ceilingFuncShift(C_param_input_col, NPU_SHIFT)) *
                          channel_size_local;
    ...
    if (slicing_success) {
      ...
      size_t bias_local_size =
          (ceilingFuncShift(C_param_weight_col, NPU_SHIFT)) * channel_size_local;
...
static void strategy_slice_on_multidim_init(KernelContext *ctx,
                                            gaddr_t *slice_global_offset,
                                            size_t *matrix_shape,
                                            size_t *slice_row_stride,
                                            KernelFmt fmt) {
  int fmt_byte_sz = get_fmt_byte_sz(fmt);
...
#endif
  MatrixLmem *tl_A = bmk1880v2_matrix_lmem_prealloc_align(ctx, &matrix_lmem[0],
                                                          0, s_A, fmt, CTRL_AL);
  MatrixLmem *tl_B = bmk1880v2_matrix_lmem_prealloc_align(
      ctx, &matrix_lmem[1], bank_size, s_B, fmt, CTRL_AL);
  MatrixGmemShape shape;
  ...
  // tl_B->start_address, global_offset_A,
  //    global_offset_B);
...
static int strategy_slice_on_multi_dimension_internal(
    KernelContext *ctx, size_t *slice_idx, size_t *slice_num,
    gaddr_t *slice_global_offset, size_t *matrix_shape,
    gaddr_t *slice_global_offset_next, size_t *matrix_shape_next,
    size_t *slice_row_stride, KernelFmt fmt) {
  int fmt_byte_sz = get_fmt_byte_sz(fmt);
  size_t row_num_A = matrix_shape[0];
  size_t col_num_A = matrix_shape[1];
  size_t col_num_B = matrix_shape[2];
  ...
  MatrixLmemShape s_next_A =
      ctx->ops->ml_default_shape(ctx, row_num_next_A, col_num_next_A, fmt);
  MatrixLmemShape s_next_B =
      ctx->ops->ml_default_shape(ctx, col_num_next_A, col_num_next_B, fmt);
  MatrixLmemShape s_A, s_B, s_C;
  ...
  MatrixTGmemStride st_A, st_C, st_B;
  st_A.row = (uint32_t)slice_row_stride[0] * fmt_byte_sz;
  st_C.row = (uint32_t)slice_row_stride[1] * fmt_byte_sz;
  st_B.row = (uint32_t)slice_row_stride[1] * fmt_byte_sz;

  MatrixLmem *tl_A = bmk1880v2_matrix_lmem_prealloc_align(
      ctx, &matrix_lmem[0], hf_bsize * cur, s_A, fmt, CTRL_AL);
  MatrixLmem *tl_next_A = bmk1880v2_matrix_lmem_prealloc_align(
      ctx, &matrix_lmem[1], hf_bsize * next, s_next_A, fmt, CTRL_AL);
  MatrixLmem *tl_B = bmk1880v2_matrix_lmem_prealloc_align(
      ctx, &matrix_lmem[2], bank_size + hf_bsize * cur, s_B, fmt, CTRL_AL);
  MatrixLmem *tl_next_B = bmk1880v2_matrix_lmem_prealloc_align(
      ctx, &matrix_lmem[3], bank_size + hf_bsize * next, s_next_B, fmt,
      CTRL_AL);
  MatrixLmem *tl_C = bmk1880v2_matrix_lmem_prealloc_align(
      ctx, &matrix_lmem[4], output_nr * bank_size, s_C, fmt, CTRL_AL);
...
#define PS32_CTRL_NULL (2) /* first one */
  ...
  if (slice_idx[1] == slice_num[2] - 1) {
    ...
    // DBG("local memory a/b/c is %u/%u/%u, store to slice_global_offset[2]
    // %lu\n",
    //    tl_A->start_address, tl_B->start_address, tl_C->start_address,
    //    slice_global_offset[2]);
  }

  bool is_alloc_from_stack = true;

  if (cur == 0) {
    bmk1880v2_lmem_free_prealloc_bf16_matrix(ctx, is_alloc_from_stack,
                                             tl_next_B);
    bmk1880v2_lmem_free_prealloc_bf16_matrix(ctx, is_alloc_from_stack, tl_B);
    bmk1880v2_lmem_free_prealloc_bf16_matrix(ctx, is_alloc_from_stack,
                                             tl_next_A);
...
static void strategy_slice_on_multi_dimension(
    KernelContext *ctx, gaddr_t global_offset_A, gaddr_t global_offset_B,
    gaddr_t global_offset_C, size_t M, size_t N, size_t K, size_t *slice_num,
    KernelFmt fmt) {
  ...
  for (slice_idx[0] = 0; slice_idx[0] < slice_num[0]; slice_idx[0]++) {
    ...
    matrix_shape_next[0] =
        M / slice_num[0] + (0 + slice_idx[0] < M % slice_num[0]);
    for (slice_idx[2] = 0; slice_idx[2] < slice_num[1]; slice_idx[2]++) {
      matrix_shape[2] = N / slice_num[1] + (slice_idx[2] < N % slice_num[1]);
      matrix_shape_next[2] =
          N / slice_num[1] + (0 + slice_idx[1] < N % slice_num[1]);
      for (slice_idx[1] = 0; slice_idx[1] < slice_num[2]; slice_idx[1]++) {
        matrix_shape[1] = K / slice_num[2] + (slice_idx[1] < K % slice_num[2]);
        matrix_shape_next[1] =
            K / slice_num[2] + (1 + slice_idx[1] < K % slice_num[2]);
        slice_global_offset[0] =
            get_slice_global_offset(global_offset_A, slice_idx[0], slice_idx[1],
                                    M, K, slice_num[0], slice_num[2], fmt);
        slice_global_offset[1] =
            get_slice_global_offset(global_offset_B, slice_idx[1], slice_idx[2],
                                    K, N, slice_num[2], slice_num[1], fmt);
        // the low 8-bits of C
        if (fmt == CVK_FMT_BF16) {
          slice_global_offset[2] = get_slice_global_offset(
              global_offset_C, slice_idx[0], slice_idx[2], M, N, slice_num[0],
              slice_num[1], fmt);
        } else {
          slice_global_offset[2] =
              get_slice_global_offset(global_offset_C, slice_idx_0, slice_idx_2,
                                      M, N, slice_num[0], slice_num[1], fmt);
          if (slice_idx[1] == slice_num[2] - 1) {
        ...
        slice_global_offset_next[0] = get_slice_global_offset(
            global_offset_A, slice_idx[0], slice_idx[1] + 1, M, K, slice_num[0],
            slice_num[2], fmt);
        slice_global_offset_next[1] = get_slice_global_offset(
            global_offset_B, slice_idx[1] + 1, slice_idx[2], K, N, slice_num[2],
            slice_num[1], fmt);
        // DBG("=>(%s)slice_global_offset[0](%lu)/slice_global_offset[1](%lu)
        // for slice_idx[1](%lu)
        // "
        //    "== 0\n"
        //    ", (%s)slice_global_offset[2](%lu) for slice_idx[1](%lu) ==
        //    slice_num[2](%lu) - 1\n"
        //    ",
        //    (%s)(next)slice_global_offset_next[0](%lu)/slice_global_offset_next[1](%lu)
        //    for " "slice_num[2](%lu) - 1 > slice_idx[1](%lu)\n" "next ctrl:%s,
        //    store ctrl:%s\n", slice_idx[1] == 0 ? "en" : " ",
        //    slice_global_offset[0], slice_global_offset[1], slice_idx[1],
        //    slice_idx[1] == slice_num[2] - 1 ? "en" : " ",
        //    slice_global_offset[2], slice_idx[1], slice_num[2], slice_num[2] -
        //    1 > slice_idx[1] ? "en" : " ", slice_global_offset_next[0],
        //    slice_global_offset_next[1], slice_num[2], slice_idx[1],
        //    (slice_idx[1] > 0) ? "CTRL_RA" : "CTRL_NULL",
        //    (slice_num[2] > 1) ? "CTRL_RA" : "CTRL_NULL");

        if (slice_idx[1] == 0) {
          strategy_slice_on_multidim_init(ctx, slice_global_offset,
                                          matrix_shape, slice_row_stride, fmt);
        }

        strategy_slice_on_multi_dimension_internal(
            ctx, slice_idx, slice_num, slice_global_offset, matrix_shape,
            slice_global_offset_next, matrix_shape_next, slice_row_stride, fmt);
      }
    }
  }
}

size_t *bmblas_gemm(KernelContext *ctx, size_t M, size_t N, size_t K,
                    uint64_t gaddr_a, uint64_t gaddr_b, uint64_t gaddr_c,
                    KernelFmt fmt) {
  ...
  // printf("strategy: %lu\n slice %lu %lu %lu\n", strategy, slice_num[0],
  // slice_num[1], slice_num[2]);
  ...
  switch (strategy) {
    ...
    case 1: {
      strategy_slice_on_M(ctx, M, N, K, gaddr_a, gaddr_b, gaddr_c, slice_num[0],
                          fmt);
    } break;
    case 2: {
      strategy_slice_on_N(ctx, M, N, K, gaddr_a, gaddr_b, gaddr_c, slice_num[1],
                          fmt);
    } break;
    case 3: {
      slice_split_strategy(ctx, M, N, K, slice_num, fmt);
      // printf("slice all, %lu %lu %lu\n", slice_num[0], slice_num[1],
      // slice_num[2]);

      strategy_slice_on_multi_dimension(ctx, gaddr_a, gaddr_b, gaddr_c, M, N, K,
                                        slice_num, fmt);
    }
    default:
      break;
  ...
  // 使用 C++ 风格分配内存并检查分配失败情况
  size_t *_slice_num = static_cast<size_t *>(malloc(slice_num_len));
  if (_slice_num == nullptr) {
    // 处理内存分配失败情况
    return nullptr;
  }
  std::memcpy(_slice_num, slice_num, 3 * sizeof(size_t));
  _slice_num[3] = strategy;
  return _slice_num;
}

size_t *cvmGemm(KernelContext *ctx, gaddr_t bottom_data_gaddr,
                gaddr_t weight_data_gaddr, gaddr_t top_data_gaddr, int in_row,
                int in_col, int out_col, KernelFmt fmt) {
  size_t *slice_num = nullptr;
  if (0) {
    // backend impelement
    cvm_fc_forward_kernel(ctx, 0, bottom_data_gaddr, weight_data_gaddr,
                          GADDR_INVALID, top_data_gaddr, in_row, in_col,
                          out_col, 0, 0, 0);
  } else {
    slice_num = bmblas_gemm(ctx, in_row, out_col, in_col, bottom_data_gaddr,
                            weight_data_gaddr, top_data_gaddr, fmt);
  }
  return slice_num;
}
...
int cvmCombinGemmI8(size_t *slice_num, uint8_t *i8_C, uint32_t *i32_C, int M,
  ...
  // 检查输入参数
  if (!slice_num || !i8_C || !i32_C || M <= 0 || N <= 0) {
    return -1;  // 返回错误码
  }

  int strategy = static_cast<int>(slice_num[3]);
  int chunks = static_cast<int>(slice_num[0] * slice_num[1] * slice_num[2]);
  ...
  if (strategy == 0 || strategy == 2) {
    ...
    for (int tiling = 0; tiling < chunks; tiling++) {
      size_t N_slice =
          N / slice_num[1] + (tiling < static_cast<int>(N % slice_num[1]));
      chunk_size = static_cast<int>(N_slice) * M;
      size = chunk_size;
      bstride = M * N;
      for (int m = 0; m < M; m++) {
        for (int n = 0; n < static_cast<int>(N_slice); n++) {
          int shift = N_slice_cnt + m * N + n;
          i32_C[shift] = (i8_C[shift + bstride * 0]) |
                         (i8_C[shift + bstride * 1] << 8) |
                         (i8_C[shift + bstride * 2] << 16) |
                         (i8_C[shift + bstride * 3] << 24);
        }
      }
      pack_shift += size;
      N_slice_cnt += static_cast<int>(N_slice);
    }
  } else if (strategy == 1) {
    int pack_shift = 0;
    for (int tiling = 0; tiling < chunks; tiling++) {
      size_t M_slice =
          M / slice_num[0] + (tiling < static_cast<int>(M % slice_num[0]));
      chunk_size = static_cast<int>(M_slice) * N;
      size = chunk_size;
      bstride = chunk_size;
      for (int i = 0; i < size; i++) {
        i32_C[pack_shift + i] =
            (i8_C[pack_shift * sizeof(int) + i + bstride * 0]) |
    ...
    // FIXME: tiling N
    int pack_shift = 0;
    for (int tiling = 0; tiling < static_cast<int>(slice_num[0]); tiling++) {
      size_t M_slice =
          M / slice_num[0] + (tiling < static_cast<int>(M % slice_num[0]));
      int size = static_cast<int>(M_slice) * N;
      int bstride = size;
      for (int i = 0; i < size; i++) {
        i32_C[pack_shift + i] =
            (i8_C[pack_shift * sizeof(int) + i + bstride * 0]) |
