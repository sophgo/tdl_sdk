  ...
  if (format == PIXEL_FORMAT_YUV_PLANAR_420) {
    ...
    for (int y = 0; y < bheight; y++) {
      for (int x = 0; x < bwidth; x++) {
        if (x < overlay_lx) {
          ...
          blend_img[y * bwidth + x] =
              (alpha * left_p + (255 - alpha) * right_p) >> 8;
        }
      }
    }
    ...
    for (int y = 0; y < uv_bheight; y++) {
      for (int x = 0; x < uv_bwidth; x++) {
        ...
        if (x < (overlay_lx / 2)) {
          blend_img[blend_u_offet] =
              left_img[lwidth * lheight + y * uv_lwidth + x];
          blend_img[blend_v_offet] =
              left_img[lwidth * lheight + luv_size + y * uv_lwidth + x];
        } else if (x > (overlay_rx / 2)) {
          int right_x = x - overlay_lx / 2;
          if (right_x >= uv_rwidth) right_x = uv_rwidth - 1;

          blend_img[blend_u_offet] =
              right_img[rwidth * rheight + y * uv_rwidth + right_x];
          blend_img[blend_v_offet] =
              right_img[rwidth * rheight + ruv_size + y * uv_rwidth + right_x];
        } else {
          // yuv alpha share
          if (wgt_mode == WGT_YUV_SHARE) {
            alpha = wgt_uv[y * overlay_uv_w + x - overlay_lx / 2];
          } else {
            alpha = wgt[bheight * overlay_w + y * overlay_uv_w + x -
                        overlay_lx / 2];
          }
          ...
          unsigned char right_u =
              right_img[rwidth * rheight + y * uv_rwidth + right_x];
          unsigned char right_v =
              right_img[rwidth * rheight + ruv_size + y * uv_rwidth + right_x];

          blend_img[blend_u_offet] =
              (alpha * left_u + (255 - alpha) * right_u) >> 8;
          blend_img[blend_v_offet] =
              (alpha * left_v + (255 - alpha) * right_v) >> 8;
        }
      }
    }
        ...
        for (int x = 0; x < bwidth; x++) {
          if (x < overlay_lx) {
            ...
            unsigned char right_pixel =
                right_img[c * rheight * rwidth + y * rwidth + (x - overlay_lx)];
...
bm_status_t tpu_blend_param_check(CVI_S32 lwidth, CVI_S32 lheight,
                                  ...
                                  PIXEL_FORMAT_E format,
                                  TPU_BLEND_WGT_MODE mode, bm_handle_t handle) {
  if (handle == NULL) {
  ...
  if (lheight < 8 || lheight > 4096 || rheight < 8 || rheight > 4096 ||
      bheight < 8 || bheight > 4096) {
    bmlib_log("BLEND", BMLIB_LOG_ERROR,
              "The img height should be [8, 4096(%d)]\r\n", lwidth);
    return BM_ERR_PARAM;
  }

  if ((format == PIXEL_FORMAT_YUV_PLANAR_420) && (lwidth % 4 != 0)) {
    bmlib_log("BLEND", BMLIB_LOG_ERROR,
              "In PIXEL_FORMAT_YUV_PLANAR_420 mode, the left image width "
              "should be 4-aligned\r\n");
    return BM_ERR_PARAM;
  }
  ...
  if ((format == PIXEL_FORMAT_YUV_PLANAR_420) && (rwidth % 4 != 0)) {
    bmlib_log("BLEND", BMLIB_LOG_ERROR,
              "In PIXEL_FORMAT_YUV_PLANAR_420 mode, the right image width "
              "should be 4-aligned\r\n");
    return BM_ERR_PARAM;
  }
  ...
  if ((format == PIXEL_FORMAT_YUV_PLANAR_420) && (bwidth % 4 != 0)) {
    bmlib_log("BLEND", BMLIB_LOG_ERROR,
              "In PIXEL_FORMAT_YUV_PLANAR_420 mode, the blend img width should "
              "be 4-aligned\r\n");
    return BM_ERR_PARAM;
  }
  ...
  if (lwidth < 8 || rwidth < 8 || bwidth < 8) {
    bmlib_log("BLEND", BMLIB_LOG_ERROR,
              "The img height should be [0, lwidth(%d)]\r\n", lwidth);
  }
...
bm_status_t tpu_2way_blending(bm_handle_t handle, CVI_S32 lwidth,
  ...
  ret = tpu_blend_param_check(lwidth, lheight, rwidth, rheight, blend_w,
                              blend_h, overlay_lx, overlay_rx,
                              (PIXEL_FORMAT_E)format, mode, handle);
