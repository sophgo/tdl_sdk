...
uint32_t OpenCVImage::getInternalType() const { return mats_[0].type(); }
...
int32_t OpenCVImage::setupMemory(uint64_t phy_addr, uint8_t* vir_addr,
                                 uint32_t length) {
  LOGI("setupMemory, phy_addr: %p, vir_addr: %p, length: %d,num_channels: %d",
       (void*)phy_addr, (void*)vir_addr, length, mats_.size());
