...
VADXOptions::VADXOptions(int sr, int dm, int sm, int mset, int msst, bool dspd,
                         bool depd, int wsm, int ststh, int sttsh, float s2nr,
                         int de, int lbtps, int latsp, int mss, int nebs,
                         int dbs, float st, int nfnus, float dt, float snt,
                         float fept, int spn, std::vector<int> spids,
                         float sntl, float snth, bool ofp, int fim, int flm)
    : sample_rate(sr),
      detect_mode(dm),
      snr_mode(sm),
...
E2EVadSpeechBufWithDoa::E2EVadSpeechBufWithDoa()
    : start_ms(0),
      end_ms(0),
      buffer(),
...
void E2EVadSpeechBufWithDoa::Reset() {
  start_ms = 0;
  end_ms = 0;
  buffer.clear();
  contain_seg_start_point = false;
...
E2EVadFrameProb::E2EVadFrameProb()
    : noise_prob(0.0),
      speech_prob(0.0),
      score(0.0),
      frame_id(0),
...
WindowDetector::WindowDetector(int window_size_ms, int sil_to_speech_time,
...
int WindowDetector::GetWinSize() { return win_size_frame; }

AudioChangeState WindowDetector::DetectOneFrame(FrameState frameState,
                                                int frame_count) {
...
int WindowDetector::FrameSizeMs() { return frame_size_ms; }

E2EVadModel::E2EVadModel()
    : vad_opts(VADXOptions()),
      windows_detector(WindowDetector(200, 150, 150, 10)),
      is_final(false),
      ...
      max_end_sil_frame_cnt_thresh(vad_opts.max_end_silence_time -
...
std::vector<std::vector<int>> E2EVadModel::operator()(
    const std::vector<std::vector<float>> &score,
    const std::vector<float> &waveform, bool is_final, bool online,
    int max_end_sil, int max_single_segment_time, float speech_noise_thres,
    int sample_rate) {
  max_end_sil_frame_cnt_thresh =
      max_end_sil - vad_opts.speech_to_sil_time_thres;
  ...
  if (output_data_buf.size() > 0) {
    for (size_t i = output_data_buf_offset; i < output_data_buf.size(); i++) {
      ...
      if (online) {
        ...
        if (!is_final && (!output_data_buf[i].contain_seg_start_point ||
                          !output_data_buf[i].contain_seg_end_point)) {
          continue;
        }
...
void E2EVadModel::AllResetDetection() {
  is_final = false;
  data_buf_start_frame = 0;
  frm_cnt = 0;
  latest_confirmed_speech_frame = 0;
...
void E2EVadModel::ResetDetection() {
  continous_silence_frame_count = 0;
  latest_confirmed_speech_frame = 0;
  lastest_confirmed_silence_frame = -1;
  confirmed_start_frame = -1;
...
void E2EVadModel::ComputeDecibel() {
  int frame_sample_length =
      int(vad_opts.frame_length_ms * vad_opts.sample_rate / 1000);
  int frame_shift_length =
      int(vad_opts.frame_in_ms * vad_opts.sample_rate / 1000);
  ...
  for (int offset = 0; offset + frame_sample_length - 1 < waveform.size();
...
void E2EVadModel::ComputeScores(const std::vector<std::vector<float>> &scores) {
  vad_opts.nn_eval_block_size = scores.size();
  frm_cnt += scores.size();
  this->scores = scores;
}

void E2EVadModel::PopDataBufTillFrame(int frame_idx) {
  int frame_sample_length =
      int(vad_opts.frame_in_ms * vad_opts.sample_rate / 1000);
  while (data_buf_start_frame < frame_idx) {
    if (data_buf_size >= frame_sample_length) {
      ...
      data_buf_size =
          data_buf_all_size - data_buf_start_frame * frame_sample_length;
    }
  }
}

void E2EVadModel::PopDataToOutputBuf(int start_frm, int frm_cnt,
  ...
  int expected_sample_number =
      int(frm_cnt * vad_opts.sample_rate * vad_opts.frame_in_ms / 1000);
  if (last_frm_is_end_point) {
    int extra_sample =
        std::max(0, int(vad_opts.frame_length_ms * vad_opts.sample_rate / 1000 -
  ...
  if (output_data_buf.size() == 0 || first_frm_is_start_point) {
    ...
    output_data_buf[output_data_buf.size() - 1].start_ms =
        start_frm * vad_opts.frame_in_ms;
    output_data_buf[output_data_buf.size() - 1].end_ms =
        output_data_buf[output_data_buf.size() - 1].start_ms;
  ...
  if (end_point_is_sent_end) {
    ...
    data_to_pop =
        int(frm_cnt * vad_opts.frame_in_ms * vad_opts.sample_rate / 1000);
  }
...
void E2EVadModel::OnSilenceDetected(int valid_frame) {
...
void E2EVadModel::OnVoiceDetected(int valid_frame) {
  latest_confirmed_speech_frame = valid_frame;
  PopDataToOutputBuf(valid_frame, 1, false, false, false);
}

void E2EVadModel::OnVoiceStart(int start_frame, bool fake_result) {
...
void E2EVadModel::OnVoiceEnd(int end_frame, bool fake_result,
                             bool is_last_frame) {
...
void E2EVadModel::MaybeOnVoiceEndIfLastFrame(bool is_final_frame,
                                             int cur_frm_idx) {
  if (is_final_frame) {
...
int E2EVadModel::GetLatency() {
  return int(LatencyFrmNumAtStartPoint() * vad_opts.frame_in_ms);
}

int E2EVadModel::LatencyFrmNumAtStartPoint() {
  int vad_latency = windows_detector.GetWinSize();
  if (vad_opts.do_extend) {
    vad_latency +=
        int(vad_opts.lookback_time_start_point / vad_opts.frame_in_ms);
  }
  return vad_latency;
}

FrameState E2EVadModel::GetFrameState(int t) {
  ...
  if (exp(speech_prob) >= exp(noise_prob) + speech_noise_thres) {
    if (cur_snr >= vad_opts.snr_thres &&
        cur_decibel >= vad_opts.decibel_thres) {
    ...
    if (noise_average_decibel < -99.9) {
      ...
      noise_average_decibel =
          (cur_decibel + noise_average_decibel *
...
int E2EVadModel::DetectCommonFrames() {
...
int E2EVadModel::DetectLastFrames() {
  if (vad_state_machine == VadStateMachine::kVadInStateEndPointDetected) {
    return 0;
  }
  for (int i = vad_opts.nn_eval_block_size - 1; i >= 0; i--) {
...
void E2EVadModel::DetectOneFrame(FrameState cur_frm_state, int cur_frm_idx,
                                 bool is_final_frame) {
  ...
  AudioChangeState state_change =
      windows_detector.DetectOneFrame(tmp_cur_frm_state, cur_frm_idx);
  ...
  if (AudioChangeState::kChangeStateSil2Speech == state_change) {
    ...
    if (vad_state_machine ==
      ...
      if ((vad_opts.detect_mode ==
               static_cast<int>(VadDetectMode::kVadSingleUtteranceDetectMode) &&
           (continous_silence_frame_count * frm_shift_in_ms >
            vad_opts.max_start_silence_time)) ||
          (is_final_frame && number_end_time_detected == 0)) {
        for (int t = lastest_confirmed_silence_frame + 1; t < cur_frm_idx;
             t++) {
          OnSilenceDetected(t);
        }
        OnVoiceStart(0, true);
        ...
        if (cur_frm_idx >= LatencyFrmNumAtStartPoint()) {
          OnSilenceDetected(cur_frm_idx - LatencyFrmNumAtStartPoint());
               ...
               VadStateMachine::kVadInStateInSpeechSegment) {
      if (continous_silence_frame_count * frm_shift_in_ms >=
          max_end_sil_frame_cnt_thresh) {
        ...
        if (continous_silence_frame_count <=
            vad_opts.lookahead_time_end_point / frm_shift_in_ms) {
          OnVoiceDetected(cur_frm_idx);
        }
      } else {
        MaybeOnVoiceEndIfLastFrame(is_final_frame, cur_frm_idx);
  ...
  if (vad_state_machine == VadStateMachine::kVadInStateEndPointDetected &&
      vad_opts.detect_mode ==
          static_cast<int>(VadDetectMode::kVadMutipleUtteranceDetectMode)) {
    ResetDetection();
  }
}