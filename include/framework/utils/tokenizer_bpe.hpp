...
public:
    ...
    int tokenizerBPE(const std::string& textFile, std::vector<std::vector<int32_t>>& tokens) {
        readVocab();
        buildPairsMap();
...
private:
    ...
    void readVocab() {
        ...
        std::cerr << "Loading vocabulary from " << encoderFile << " ..." << std::endl;
        std::string line;
    ...
    std::unordered_map<std::pair<std::string, std::string>, int, pair_hash> createPairsMap(
    ...
    void processSentences(std::vector<std::vector<int32_t>>& tokens, const std::vector<std::string>& text) {
        std::string special = "[@#$%^&*!]";
        std::regex pattern(
            special +
        ...
        for (int j = 0; j < text.size(); j++) {
            ...
            std::transform(line.begin(), line.end(), line.begin(), ::tolower);  // convert all to lowercase
    ...
    void mergePairs(std::vector<std::string>& word, std::set<std::pair<std::string, std::string>>& pairs) {
        while (true) {
