...
class DetectionTestSuite : public CVI_TDLModelTestSuite {
 ...
 protected:
  ...
  void runAccuracyWithRuntimeMemory() {
    ...
    std::shared_ptr<BaseMemoryPool> pool =
    ...
    for (uint32_t i = 0; i < mem_sizes.size(); i++) {
      ...
      if (!mem_block) {
        LOGE("Failed to allocate memory of %u bytes", mem_sizes[i]);
        return;
      }
...
#if defined(__CV184X__)
TEST_F(DetectionTestSuite, DISABLED_accuracy_runtime_memory) {
  if (CVI_TDLTestContext::getInstance().getTestFlag() ==
      TestFlag::GENERATE_FUNCTION_RES) {
    GTEST_SKIP()
        << "runtime memory regression does not support result generation";
  }
  runAccuracyWithRuntimeMemory();
}
#endif
