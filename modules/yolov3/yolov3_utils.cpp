...
void GetYoloDetections(YOLOLayer l, int w, int h, int net_width, int net_height, float threshold, int relative, detection *dets, YOLOParamter yolov3_param, vector<int> m_mask) {
    ...
    for (i = 0; i < layer_w * layer_h; ++i) {
        ...
        for (n = 0; n < yolov3_param.m_anchor_nums; ++n) {
            ...
            dets[count].bbox = GetYoloBox(predictions, yolov3_param.m_biases, biases_idx, box_index, col, row, layer_w, layer_h, net_width, net_height, layer_w * layer_h);
...
detection *GetNetworkBoxes(YOLOLayer net_output, int classes, int w, int h, float threshold, int relative,
                           int *num, YOLOParamter yolo_param, int index) {
