...
int Yolov3::inference(VIDEO_FRAME_INFO_S *srcFrame, cvi_object_t *obj, cvi_obj_det_type_t det_type) {
...
void Yolov3::outputParser(cvi_object_t *obj, cvi_obj_det_type_t det_type) {
  ...
  for (int i = 0; i < m_output_num; i++) {
      CVI_TENSOR *out = CVI_NN_GetTensorByName(output_name[i].c_str(), mp_output_tensors, m_output_num);
  ...
  for (size_t i = 0; i < net_outputs.size(); i++) {
    ...
    detection *dets = GetNetworkBoxes(net_outputs.at(i), m_yolov3_param.m_classes, yolov3_w, yolov3_h,
  ...
  getYOLOResults(total_dets, total_boxes, m_yolov3_param.m_threshold, yolov3_h, yolov3_w, results, det_type);
    ...
    for (int b = 0; b < m_yolov3_param.m_batch; ++b) {
        for (int p = 0; p < w * h; ++p) {
            for (int n = 0; n < m_yolov3_param.m_anchor_nums; ++n) {
                int obj_index  = EntryIndex(w, h, m_yolov3_param.m_classes, b, n * w * h + p, m_yolov3_param.m_coords, output_size);
                ActivateArray(data + obj_index, 1, true);
                float objectness = data[obj_index];

                if (objectness >= m_yolov3_param.m_threshold) {
                    int box_index = EntryIndex(w, h, m_yolov3_param.m_classes, b, n * w * h + p, 0, output_size);
                    ActivateArray(data + box_index, 1, true);
                    ActivateArray(data + box_index + (w * h), 1, true);

                    for (int j = 0; j < m_yolov3_param.m_classes; ++j) {
                        int class_index = EntryIndex(w, h, m_yolov3_param.m_classes, b, n* w * h + p, 4 + 1 + j, output_size);
