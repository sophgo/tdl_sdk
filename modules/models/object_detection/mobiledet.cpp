...
static void decode_box(const float *const box, const AnchorBox &anchor, cvtdl_bbox_t &det) {
...
void clip_bbox(const size_t image_width, const size_t image_height, cvtdl_bbox_t &box) {
...
MobileDetV2Detection::MobileDetV2Detection(MobileDetV2Detection::Category category, float iou_thresh)
    : m_model_config(CvimodelInfo::create_config(category)), m_iou_threshold(iou_thresh) {
...
int32_t MobileDetV2Detection::onModelOpened() {
  ...
  if (not_named_tensor > 0) {
    ...
    for (size_t j = 0; j < num_output; j++) {
      ...
      if (channel == num_cls * num_per_grid) {
        ...
        LOGE("unexpected branch,channel:%d,name:%s", channel, output_layers[j].c_str());
      }
    }
  ...
  for (auto pair : m_model_config.class_out_names) {
    ...
    m_model_config.class_dequant_thresh[stride] = net_->getTensorInfo(name).qscale;
  }

  for (auto pair : m_model_config.bbox_out_names) {
    int stride = pair.first;
    string name = pair.second;
    m_model_config.bbox_dequant_thresh[stride] = net_->getTensorInfo(name).qscale;
  }
...
void MobileDetV2Detection::generate_dets_for_tensor(std::map<int, std::vector<cvtdl_bbox_t>> &det_vec, float class_dequant_thresh,
...
void MobileDetV2Detection::generate_dets_for_each_stride(std::map<int, std::vector<cvtdl_bbox_t>>& det_vec) {
...
void MobileDetV2Detection::get_raw_outputs(std::vector<pair<int8_t *, size_t>> *cls_tensor_ptr,
  ...
  for (auto stride : m_model_config.strides) {
    {
      const TensorInfo &info = net_->getTensorInfo(m_model_config.class_out_names[stride]);
      cls_tensor_ptr->push_back({reinterpret_cast<int8_t *>(info.sys_mem), info.tensor_elem}); 
    }

    {
      const TensorInfo &info = net_->getTensorInfo(m_model_config.bbox_out_names[stride]);
      bbox_tensor_ptr->push_back({reinterpret_cast<int8_t *>(info.sys_mem), info.tensor_elem});
    }

    {
      ...
      objectness_tensor_ptr->push_back({reinterpret_cast<int8_t *>(info.sys_mem), info.tensor_elem});
    }
  }
}

void MobileDetV2Detection::select_classes(const std::vector<uint32_t>& selected_classes) {
  m_filter.reset();
