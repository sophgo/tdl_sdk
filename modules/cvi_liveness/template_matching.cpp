...
cv::Mat template_matching(const cv::Mat &crop_rgb_frame, const cv::Mat &ir_frame, cv::Rect box) {
    ...
    cv::Rect ir_crop(adj_width_start, adj_height_start, adj_width_end-adj_width_start, adj_height_end-adj_height_start);
    ...
    cv::resize(crop_rgb_frame, crop_rgb_frame_scaled, cv::Size(int(CROP_RGB_SIZE), int(CROP_RGB_SIZE)));
    cv::resize(ir_frame_small, ir_frame_small_scaled, cv::Size(int(round(ir_frame_small.cols / scale_w)), int(round(ir_frame_small.rows / scale_h))));
...
vector<cv::Mat> TTA_9_cropps(cv::Mat image) {
    ...
    for (int start_index = 0; start_index < starts.size(); start_index++) {
        cv::Mat image_;
        image.copyTo(image_);
