...
void bench_mark_all(std::string bench_path, std::string image_root, std::string res_path,
                    cvitdl_handle_t tdl_handle, VIDEO_FRAME_INFO_S &fdFrame, float *awb_args) {
  ...
  while (getline(file, line)) {
    if (!line.empty()) {
      ...
      while (ss >> image_name) {
        ...
        CVI_TDL_SetSkipVpssPreprocess(tdl_handle, CVI_TDL_SUPPORTED_MODEL_ISP_IMAGE_CLASSIFICATION,
                                      true);
        // printf("run inference for %s\n",image_name.c_str());
        cvtdl_isp_meta_t isparg = {awb_args[0], awb_args[1], awb_args[2]};
        int ret = CVI_TDL_Isp_Image_Classification(tdl_handle, &fdFrame, &meta, &isparg);
        if (ret != CVI_SUCCESS) {
          CVI_TDL_Free(&meta);
...
int main(int argc, char *argv[]) {
  ...
  ret = CVI_TDL_Set_Isp_Image_Cls_Param(tdl_handle, &p_preprocess_cfg);
  ...
  ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_ISP_IMAGE_CLASSIFICATION,
                          model_path.c_str());
  if (ret != CVI_SUCCESS) {
  ...
  float awb_args[3] = {1024 * 2.06111, 1024 * 1.0, 1024 * 1.93158};
  // cvtdl_isp_meta_t isparg = {awb_args[0], awb_args[1], awb_args[2]};
  bench_mark_all(bench_path, image_root, res_path, tdl_handle, fdFrame, awb_args);
