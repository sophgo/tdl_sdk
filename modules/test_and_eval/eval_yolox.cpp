...
CVI_S32 get_yoloX_det(std::string img_path, cviai_handle_t ai_handle, VIDEO_FRAME_INFO_S* fdFrame,
  ...
  while (getline(file, line)) {
    if (!line.empty()) {
      ...
      while (ss >> image_name) {
        ...
        CVI_S32 ret = get_yoloX_det(image_root + image_name, ai_handle, &fdFrame, &obj_meta);
        if (ret != CVI_SUCCESS) {
...
int main(int argc, char* argv[]) {
  ...
  printf("start yolox algorithm config \n");
  // setup yolox param
  YoloAlgParam p_yolovx_param;
  p_yolovx_param.cls = 80;

  printf("setup yolox param \n");
  ret = CVI_AI_Set_YOLOX_Param(ai_handle, &p_preprocess_cfg, &p_yolovx_param);
