...
CVI_S32 get_yolo_det(std::string img_path, cvitdl_handle_t tdl_handle, VIDEO_FRAME_INFO_S* fdFrame,
  ...
  while (getline(file, line)) {
    if (!line.empty()) {
      ...
      while (ss >> image_name) {
        ...
        CVI_S32 ret = get_yolo_det(image_root + image_name, tdl_handle, &fdFrame, &obj_meta);
        if (ret != CVI_SUCCESS) {
