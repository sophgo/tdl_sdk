...
/**
  ...
  *         when entry = 0,  geting the anchor in outputs index, when entry = 4, geting the confidence of this anchor
  *         entry = 5, get the index of C1, 
...
box GetYoloBox(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride, bool fast_exp) ;
...
void FillNetworkBoxes(YOLOLayer net_output, int w, int h, float threshold, int relative, detection *dets, YOLOParamter yolo_param);
