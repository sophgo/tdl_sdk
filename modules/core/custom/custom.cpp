...
int Custom::initAfterModelOpened() {
  ...
  if (m_factor.size() == 1) {
    float q_factor = m_factor[0] * 128 / m_quant_threshold;
    float q_mean = (-1) * m_mean[0] * 128 / m_quant_threshold;
    for (int i = 0; i < 3; i++) {
      factor[i] = q_factor;
      mean[i] = q_mean;
    }
    ...
    for (int i = 0; i < 3; i++) {
      float q_factor = m_factor[i] * 128 / m_quant_threshold;
      float q_mean = (-1) * m_mean[i] * 128 / m_quant_threshold;
      factor[i] = q_factor;
      mean[i] = q_mean;
    }
  } else {
...
int Custom::getOutputTensor(const char *tensorName, int8_t *tensor, uint32_t *tensor_count,
                            uint16_t *unit_size) {
  CVI_TENSOR *out = CVI_NN_GetTensorByName(tensorName, mp_input_tensors, m_input_num);
  if (out == NULL) {
