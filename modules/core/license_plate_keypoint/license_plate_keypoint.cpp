...
int LicensePlateKeypoint::inference(VIDEO_FRAME_INFO_S *frame, cvtdl_object_t *vehicle_plate_meta) {
  if (frame->stVFrame.enPixelFormat != PIXEL_FORMAT_RGB_888 &&
    ...
    LOGE("Error: pixel format not match PIXEL_FORMAT_RGB_888 or PIXEL_FORMAT_BGR_888");
  ...
  for (size_t n = 0; n < vehicle_plate_meta->size; n++) {
    ...
    cur_obj_info = info_extern_crop_resize_img(frame->stVFrame.u32Width, frame->stVFrame.u32Height,
                                               &vehicle_plate_meta->info[n], 0.15, 0.35);
...
int LicensePlateKeypoint::outputParser(cvtdl_vehicle_meta *v_meta) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    v_meta->license_pts.y[i] =
        static_cast<float>(cur_obj_info.bbox.y1 + out_pts[2 * i + 1] * frame_h);
  }
  return CVI_SUCCESS;
}
}  // namespace cvitdl