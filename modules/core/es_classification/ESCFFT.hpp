 ...
 public:
  void init(size_t size) {
    if (_size != size) {
      ...
      wt(size4, _ip.data(), _w.data());
      ct(size4, _ip.data(), _w.data() + size4);
    }
  }
  ...
  void ifft(float *data, const float *re, const float *im) {
    ...
    // Convert back to split-complex
    ScaleBuffer(data, _buffer.data(), 2.0 / static_cast<float>(_size), _size);
  }
