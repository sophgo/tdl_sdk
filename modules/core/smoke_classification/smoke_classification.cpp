...
SmokeClassification::SmokeClassification() : Core(CVI_MEM_SYSTEM) {}

int SmokeClassification::inference(VIDEO_FRAME_INFO_S *frame, cvtdl_face_t *meta) {
...
void SmokeClassification::prepareInputTensor(cv::Mat &input_mat) {
  const TensorInfo &tinfo = getInputTensorInfo(0);
  float *input_ptr = tinfo.get<float>();
  cv::Mat temp_mat;
  input_mat.convertTo(temp_mat, CV_32FC1, SMOKECLASSIFICATION_SCALE * 1.0, 0);
