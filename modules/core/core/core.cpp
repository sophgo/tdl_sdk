...
Core::Core(CVI_MEM_TYPE_E input_mem_type) {
  ...
  std::fill(std::begin(first_in_pre_param.factor),
            std::end(first_in_pre_param.factor), 0.0f);
  std::fill(std::begin(first_in_pre_param.mean),
            std::end(first_in_pre_param.mean), 0.0f);
...
int Core::modelClose() {
  ...
  if (mp_mi->conf.input_mem_type == CVI_MEM_SYSTEM) {
    ...
    for (int32_t i = 0; i < mp_mi->out.num; i++) {
      ...
      bm_status_t status =
          bm_mem_unmap_device_mem(bm_handle, mp_mi->out.raw_pointer[i], size);
...
float Core::getInputQuantScale(size_t index) {
  return getInputTensorInfo(index).qscale;
}

float Core::getInputQuantScale(const std::string &name) {
  return getInputTensorInfo(name).qscale;
}
...
int Core::run(std::vector<VIDEO_FRAME_INFO_S *> &frames) {
  ...
  if (mp_mi->conf.input_mem_type == CVI_MEM_DEVICE) {
    if (m_skip_vpss_preprocess) {
      ...
      for (uint32_t i = 0; i < frames.size(); i++) {
        ...
        int vpssret = mp_vpss_inst->vpssPreprocess(
            frames[i], f, preprocess_params_[i], m_vpss_timeout);
        if (vpssret != 0) {
          releaseVideoFrame(f);
          ...
          dstFrames.push_back(
              std::shared_ptr<VIDEO_FRAME_INFO_S>(f, releaseVideoFrame));
        }
      }
  ...
  ret = bmrt_launch_tensor_ex(
      mp_mi->handle, mp_mi->net_names[0], mp_mi->in.tensors.data(),
  ...
  if (ret == true) {
    ...
    for (auto i = 0; i < mp_mi->out.num; i++) {
      bm_status_t status =
          bm_mem_invalidate_device_mem(bm_handle, &(out_tensor[i].device_mem));
      if (CVI_TDL_SUCCESS != status) {
        cleanupHandle();
        return CVI_TDL_ERR_OPEN_MODEL;
      }
...
int Core::registerFrame2Tensor(std::vector<T> &frames) {
  ...
  for (uint32_t i = 0; i < (uint32_t)frames.size(); i++) {
    auto frame = frames[i];
    if (!raw) {
      if (input_w != frame->stVFrame.u32Width ||
          input_h != frame->stVFrame.u32Height) {
        LOGE("input frame shape[%u,%u] not equal with tensor input[%u,%u]",
             frame->stVFrame.u32Width, frame->stVFrame.u32Height, input_w,
             input_h);
        return CVI_TDL_FAILURE;
      }
    }
    ...
    if (mp_mi->conf.input_mem_type == CVI_MEM_DEVICE) {
      ...
      bm_status_t status = bm_mem_flush_device_mem(
          bm_handle, &(mp_mi->in.tensors[i].device_mem));
      ...
      if (input_width % ALIGN_SIZE == 0) {
        ...
        uint64_t image_bgr_devaddr =
            bm_mem_get_device_addr(mp_mi->in.tensors[i].device_mem);
        ...
        for (int c = 0; c < 3; c++) {
          for (int h = 0; h < input_height; h++) {
            memcpy(register_temp_buffer + c * input_height_squared +
                       h * input_width,
                   frame->stVFrame.pu8VirAddr[c] + h * align_length,
                   input_width);
          }
        }
        bm_device_mem_t dst_addr =
            bm_mem_from_device(image_bgr_devaddr, image_size);
        bm_memcpy_s2d_partial(bm_handle, dst_addr,
                              (void *)(register_temp_buffer), image_size);
      }
...
int Core::modelOpen(const char *filepath) {
  ...
  if (mp_mi->conf.input_mem_type == CVI_MEM_DEVICE) {
    ...
    for (auto i = 0; i < mp_mi->in.num; i++) {
      bmrt_tensor(&in_tensor[i], mp_mi->handle, net_info->input_dtypes[i],
                  stage.input_shapes[i]);
    }
  }

  for (auto i = 0; i < mp_mi->out.num; i++) {
    bmrt_tensor(&out_tensor[i], mp_mi->handle, net_info->output_dtypes[i],
                stage.output_shapes[i]);
    bm_status_t status = bm_mem_mmap_device_mem(
        bm_handle, &out_tensor[i].device_mem,
  ...
  if (mp_mi->conf.input_mem_type == CVI_MEM_SYSTEM) {
    for (auto i = 0; i < mp_mi->in.num; i++) {
      mp_mi->in.raw_pointer.push_back(
          new char[bmrt_tensor_bytesize(&in_tensor[i])]);
    }
  }
...
void Core::setupInputTensorInfo(
    const bm_net_info_t *net_info, CvimodelInfo *p_mi,
  ...
  for (int32_t i = 0; i < p_mi->in.num; i++) {
    ...
    tinfo.shape.dim = std::vector<int>(
        stages.input_shapes[i].dims,
        stages.input_shapes[i].dims + stages.input_shapes[i].num_dims);

    tinfo.tensor_elem = bmrt_shape_count(&stages.input_shapes[i]);
    tinfo.tensor_size =
        tinfo.tensor_elem * bmrt_data_type_size(net_info->input_dtypes[i]);
    ...
    tensor_info.emplace(
        std::pair<std::string, TensorInfo>(tinfo.tensor_name, tinfo));
    LOGI("input:%s,elem_num:%d,elem_size:%d\n", tinfo.tensor_name.c_str(),
         int(tinfo.tensor_elem), int(tinfo.tensor_size));
  }
}

void Core::setupOutputTensorInfo(
    const bm_net_info_t *net_info, CvimodelInfo *p_mi,
    std::map<std::string, TensorInfo> &tensor_info) {
...
int Core::run(std::vector<VIDEO_FRAME_INFO_S *> &frames) {
    ...
    if (m_skip_vpss_preprocess) {
      ...
      for (uint32_t i = 0; i < frames.size(); i++) {
        ...
        if (vpssret != CVI_TDL_SUCCESS) {
          ...
          dstFrames.push_back(std::shared_ptr<VIDEO_FRAME_INFO_S>(
              {f, [this](VIDEO_FRAME_INFO_S *f) {
  ...
  model_timer_.TicToc("vpss");
  if (ret == CVI_TDL_SUCCESS) {
    int rcret = CVI_NN_Forward(mp_mi->handle, mp_mi->in.tensors, mp_mi->in.num,
...
int Core::registerFrame2Tensor(std::vector<T> &frames) {
  ...
  for (uint32_t i = 0; i < (uint32_t)frames.size(); i++) {
    ...
    if (aligned_input == true) {
      ...
      ret = CVI_NN_FeedTensorWithFrames(
          mp_mi->handle, mp_mi->in.tensors, frame_type_, CVI_FMT_INT8,
...
int Core::modelOpen(const char *filepath) {
  ...
  CLOSE_MODEL_IF_TPU_FAILED(
      CVI_NN_GetInputOutputTensors(mp_mi->handle, &mp_mi->in.tensors,
                                   &mp_mi->in.num, &mp_mi->out.tensors,
                                   &mp_mi->out.num),
  ...
  if (true == aligned_input) {
    for (int32_t i = 0; i < mp_mi->in.num; i++)
      CLOSE_MODEL_IF_TPU_FAILED(
          CVI_NN_SetTensorPhysicalAddr(&mp_mi->in.tensors[i], (uint64_t)0),
...
int Core::modelOpen(const int8_t *buf, uint32_t size) {
  ...
  CLOSE_MODEL_IF_TPU_FAILED(
      CVI_NN_RegisterModelFromBuffer(buf, size, &mp_mi->handle),
...
const TensorInfo &Core::getOutputTensorInfo(size_t index) {
  size_t cur = 0;
  for (auto iter = m_output_tensor_info.begin();
       iter != m_output_tensor_info.end(); iter++) {
    if (cur == index) {
      return iter->second;
    }
    cur++;
...
const TensorInfo &Core::getInputTensorInfo(size_t index) {
  size_t cur = 0;
  for (auto iter = m_input_tensor_info.begin();
       iter != m_input_tensor_info.end(); iter++) {
    if (cur == index) {
      return iter->second;
    }
    cur++;
...
size_t Core::getNumOutputTensor() const {
  return static_cast<size_t>(mp_mi->out.num);
}
...
CVI_SHAPE Core::getInputShape(size_t index) {
  return getInputTensorInfo(index).shape;
}

CVI_SHAPE Core::getOutputShape(size_t index) {
  return getOutputTensorInfo(index).shape;
}

CVI_SHAPE Core::getInputShape(const std::string &name) {
  return getInputTensorInfo(name).shape;
}

CVI_SHAPE Core::getOutputShape(const std::string &name) {
  return getOutputTensorInfo(name).shape;
}

size_t Core::getOutputTensorElem(size_t index) {
  return getOutputTensorInfo(index).tensor_elem;
}
