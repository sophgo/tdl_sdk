...
bool VpssPreprocessor::init() {
  ...
  uint32_t width = 100;
  uint32_t height = 100;
  VPSS_GRP_DEFAULT_HELPER2(&vpss_grp_attr, width, height, VI_PIXEL_FORMAT, device_);
...
std::shared_ptr<BaseImage> VpssPreprocessor::preprocess(
  ...
  int32_t ret = vpss_image->prepareFrameInfo(params.dstWidth, params.dstHeight,
                                             params.dstImageFormat, params.dstPixDataType);
  if (ret != 0) {
    LOGE("VPSSImage prepareFrameInfo failed!\n");
    return nullptr;
  }
  if (memory_pool == nullptr) {
    if (use_vb_pool_) {
      memory_block = CviMemoryPool::create_vb(vpss_image->getImageByteSize());
    } else {
      memory_block = CviMemoryPool::create(vpss_image->getImageByteSize());
    }
  } else {
  ...
  ret = prepareVPSSParams(image, params);
  if (ret != CVI_SUCCESS) {
    LOGE("prepareVPSSParams failed with %#x\n", ret);
    return nullptr;
  }
  VIDEO_FRAME_INFO_S* output_frame = static_cast<VIDEO_FRAME_INFO_S*>(vpss_image->getData());
  const VIDEO_FRAME_INFO_S* input_frame = static_cast<const VIDEO_FRAME_INFO_S*>(image->getData());

  auto vir_addrs = vpss_image->getVirtualAddress();
  ...
  for (int i = 0; i < vpss_image->getPlaneNum(); i++) {
    LOGI("vir_addrs[%d]: %p, output_frame->stVFrame.pu8VirAddr[%d]: %p", i, vir_addrs[i], i,
         output_frame->stVFrame.pu8VirAddr[i]);
    output_frame->stVFrame.pu8VirAddr[i] = vir_addrs[i];
  }
  LOGI("CVI_VPSS_GetChnFrame done");
  return vpss_image;
}

int32_t VpssPreprocessor::prepareVPSSParams(const std::shared_ptr<BaseImage>& src_image,
                                            const PreprocessParams& params) {
  VPSS_GRP_ATTR_S vpss_grp_attr;
  VPSS_CROP_INFO_S vpss_chn_crop_attr;
  VPSS_CHN_ATTR_S vpss_chn_attr;
  bool is_ok =
      generateVPSSParams(src_image, params, vpss_grp_attr, vpss_chn_crop_attr, vpss_chn_attr);
  if (!is_ok) {
...
bool VpssPreprocessor::generateVPSSParams(const std::shared_ptr<BaseImage>& src_image,
                                          const PreprocessParams& params,
                                          VPSS_GRP_ATTR_S& vpss_grp_attr,
                                          VPSS_CROP_INFO_S& vpss_chn_crop_attr,
  ...
  const VPSSImage* vpss_image_src = dynamic_cast<const VPSSImage*>(src_image.get());
  VPSS_GRP_DEFAULT_HELPER2(&vpss_grp_attr, vpss_image_src->getWidth(), vpss_image_src->getHeight(),
                           vpss_image_src->getPixelFormat(), device_);

  PIXEL_FORMAT_E dst_format =
      vpss_image_src->convertPixelFormat(params.dstImageFormat, params.dstPixDataType);
  VPSS_CHN_SQ_HELPER(&vpss_chn_attr, params.dstWidth, params.dstHeight, dst_format, params.mean,
                     params.scale, false);
  ...
  if (params.cropWidth > 0 && params.cropHeight > 0) {
    ...
    vpss_chn_crop_attr.stCropRect = {params.cropX, params.cropY, params.cropWidth,
                                     params.cropHeight};
  }
  return true;
}

std::shared_ptr<BaseImage> VpssPreprocessor::resize(const std::shared_ptr<BaseImage>& image,
...
std::shared_ptr<BaseImage> VpssPreprocessor::crop(const std::shared_ptr<BaseImage>& image, int x,
