...
CVI_S32 CVI_TDL_ReadImage(const char *filepath, VIDEO_FRAME_INFO_S *frame, PIXEL_FORMAT_E format) {
  ...
  try {
    ...
    if (CREATE_ION_HELPER(frame, img.cols, img.rows, format, "cvitdl/image") != CVI_TDL_SUCCESS) {
...
CVI_S32 CVI_TDL_ReadImage_Resize(const char *filepath, VIDEO_FRAME_INFO_S *frame,
...
CVI_S32 CVI_TDL_ReleaseImage(VIDEO_FRAME_INFO_S *frame) {
