...
int DrawPose17(const cvtdl_object_t *obj, VIDEO_FRAME_INFO_S *frame, float score_threshold) {
  ...
  for (uint32_t i = 0; i < obj->size; i++) {
    for (int k = 0; k < 19; k++) {
      ...
      if (obj->info[i].pedestrian_properity->pose_17.score[kps1] < score_threshold ||
          obj->info[i].pedestrian_properity->pose_17.score[kps2] < score_threshold)
        continue;

      int x1 = (int)obj->info[i].pedestrian_properity->pose_17.x[kps1];
      int y1 = (int)obj->info[i].pedestrian_properity->pose_17.y[kps1];

      int x2 = (int)obj->info[i].pedestrian_properity->pose_17.x[kps2];
      int y2 = (int)obj->info[i].pedestrian_properity->pose_17.y[kps2];

      uint8_t color_y = static_cast<uint8_t>(GetYuvColor(PLANE_Y, &color));
      cv::line(image, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(color_y), 2);
    }
  }

  // skip to draw uv plane (to speed up)
