...
static CVI_S32 update_output_state(const cvitdl_handle_t tdl_handle, face_capture_t *face_cpt_info);
static CVI_S32 extract_cropped_face(const cvitdl_handle_t tdl_handle,
                                    face_capture_t *face_cpt_info);
// static CVI_S32 capture_face_with_vpss(const cvitdl_handle_t tdl_handle, face_capture_t
// *face_cpt_info,
//                                       VIDEO_FRAME_INFO_S *frame, cvtdl_face_t *face_meta);
static void face_quality_assessment(VIDEO_FRAME_INFO_S *frame, cvtdl_face_t *face, bool *skip,
                                    quality_assessment_e qa_method, float thr_laplacian);

/* face capture functions (helper) */
static void set_skipFQsignal(face_capture_t *face_cpt_info, cvtdl_face_t *face_info, bool *skip);
// static bool is_qualified(face_capture_t *face_cpt_info, cvtdl_face_info_t *face_info,
//                          float current_quality);
// static CVI_S32 update_output_num(const cvitdl_handle_t tdl_handle,face_capture_t *face_cpt_info,
// cvtdl_tracker_t *tracker_meta);
...
static float get_score(cvtdl_face_info_t *face_info, uint32_t img_w, uint32_t img_h,
                       bool fl_model) {
...
static CVI_S32 update_output_state(cvitdl_handle_t tdl_handle, face_capture_t *face_cpt_info) {
  for (uint32_t j = 0; j < face_cpt_info->size; j++) {
...
static void encode_img2jpg(VENC_CHN VeChn, VIDEO_FRAME_INFO_S *src_frame,
                           VIDEO_FRAME_INFO_S *crop_frame, cvtdl_image_t *dst_image) {
...
static void set_skipFQsignal(face_capture_t *face_cpt_info, cvtdl_face_t *face_meta, bool *skip) {
...
// bool _FaceCapture_FilterWithLandmark(const cvitdl_handle_t tdl_handle, face_capture_t
// *face_cpt_info,
//                                      int data_idx) {
...
//   }
//   // return false;
//   cvtdl_image_t *p_img = &face_cpt_info->data[data_idx].image;
//   VIDEO_FRAME_INFO_S frame;
...
//   //
//   cvtdl_face_t obj_meta = {0};
//   ret = CVI_TDL_FLDet3(tdl_handle, &frame, &obj_meta);
//   // restore cropped image landmark to original image space
//   cvtdl_bbox_t crop_box = face_cpt_info->data[data_idx].crop_box;
//   float boxw = crop_box.x2 - crop_box.x1;
//   float boxh = crop_box.y2 - crop_box.y1;
...
// CVI_S32 update_output_num(const cvitdl_handle_t tdl_handle,face_capture_t *face_cpt_info,
// cvtdl_tracker_t *tracker_meta){
...
//   CVI_TDL_DeepSORT_UpdateOutNum(tdl_handle,tracker_meta);
//   return 0;
// }
